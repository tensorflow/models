...
data_augmentation_options that have been used and should be working :
random_image_scale
...
def random_horizontal_flip(image,
  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image, boxes]):
    ...
    print('random_horizontal_flip', tf.stack([image]))
    # tf.summary.image('random_horizontal_flip', tf.stack([image]), 10)
    result.append(image)
...
def random_pixel_value_scale(image, minval=0.9, maxval=1.1, seed=None):
  ...
  with tf.name_scope('RandomPixelValueScale', values=[image]):
    ...
    image_converted = tf.image.convert_image_dtype(image, orig_dtype, saturate=True)

    print('random_pixel_value_scale', tf.stack([image_converted]))
    # tf.summary.image('random_pixel_value_scale', tf.stack([image_converted]), 10)
    return image_converted

  print('random_pixel_value_scale', tf.stack([image]))
  # tf.summary.image('random_pixel_value_scale', tf.stack([image]), 10)
  return image
...
def random_image_scale(image,
  ...
  with tf.name_scope('RandomImageScale', values=[image]):
    ...
    print('random_image_scale', tf.stack([image]))
    # tf.summary.image('random_image_scale', tf.stack([image]), 10)
    result.append(image)
    if masks:
...
def random_adjust_brightness(image, max_delta=0.2):
  ...
  with tf.name_scope('RandomAdjustBrightness', values=[image]):
    ...
    print('random_adjust_brightness', tf.stack([image]))
    # tf.summary.image('random_adjust_brightness', tf.stack([image]))
    return image
...
def random_adjust_contrast(image, min_delta=0.8, max_delta=1.25):
  ...
  with tf.name_scope('RandomAdjustContrast', values=[image]):
    ...
    print('random_adjust_contrast', tf.stack([image]))
    # tf.summary.image('random_adjust_contrast', tf.stack([image]), 10)
    return image
...
def random_adjust_hue(image, max_delta=0.02):
  ...
  with tf.name_scope('RandomAdjustHue', values=[image]):
    ...
    print('random_adjust_hue', tf.stack([image]))
    # tf.summary.image('random_adjust_hue', tf.stack([image]), 10)
    return image
...
def random_adjust_saturation(image, min_delta=0.8, max_delta=1.25):
  ...
  with tf.name_scope('RandomAdjustSaturation', values=[image]):
    ...
    print('random_adjust_saturation', tf.stack([image]))
    # tf.summary.image('random_adjust_saturation', tf.stack([image]), 10)
    return image
...
def random_crop_pad_image(image,
  ...
  print('random_crop_pad_image', tf.stack([padded_image]))
  # tf.summary.image('random_crop_pad_image', tf.stack([padded_image]), 10)
...
def random_black_patches(image,
  ...
  with tf.name_scope('RandomBlackPatchInImage', values=[image]):
    ...
    print('random_black_patches', tf.stack([image]))
    tf.summary.image('random_black_patches', tf.stack([image]), 10)
    return image
...
def subtract_channel_mean(image, means=None):
  ...
  with tf.name_scope('SubtractChannelMean', values=[image, means]):
    ...
    print('subtract_channel_mean', tf.stack([image]))
    tf.summary.image('subtract_channel_mean', tf.stack([image]), 10)
    return image - [[means]]
