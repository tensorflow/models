...
def AddCrossEntropy(batch_size, n):
  ...
  for beam_id in range(batch_size):
    ...
    cross_entropies.append(cf.cond(
        tf.greater_equal(beam_gold_slot[0], 0), _ComputeCrossEntropy, _Pass))
...
class StructuredGraphBuilder(graph_builder.GreedyParser):
  ...
  def _BuildSequence(self,
    ...
    def Advance(features, state, concat_scores, alive_steps):
      scores = self._BuildNetwork(features,
                                  return_average=use_average)['logits']
      features, state, alive = (gen_parser_ops.beam_parser(state, scores,
                                                           self._feature_size))
      concat_scores = tf.concat(0, [concat_scores, scores],
                                name='concat_scores')
      return (alive,) + tuple(features) + (state, concat_scores, alive_steps +
                                           tf.cast(alive, tf.int32))

    alive = tf.constant(True, tf.bool, [batch_size])
    alive_steps = tf.constant(0, tf.int32, [batch_size])
    concat_scores = tf.constant([[]], dtype=tf.float32,
    ...
    for step in range(max_steps):
      ...
      t = cf.cond(tf.reduce_any(alive), AdvanceFunc, PassFunc)
      alive = t[0]
      features = list(t[1:-3])
      alive_steps = t[-1]
      concat_scores = t[-2]
    ...
    # Link to the final nodes/values of ops that have passed through Cond:
    return {'alive': alive,
