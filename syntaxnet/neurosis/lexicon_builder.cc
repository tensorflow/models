...
class LexiconBuilder : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext *context) override {
    ...
    StringToStringMap tag_to_category;
    ...
    while ((document = corpus.Read()) != NULL) {
      ...
      for (int t = 0; t < document->token_size(); ++t) {
        ...
        // Add mapping from tag to category if the tag has never been
        // encountered so far.
        if (tag_to_category.pair_size() < tags.Size()) {
          StringToStringPair *pair = tag_to_category.add_pair();
          pair->set_key(token.tag());
          pair->set_value(token.category());
        } else {
    ...
    TF_CHECK_OK(WriteStringToFile(
        ...
        tag_to_category.SerializeAsString()));
  }
