...
from official.mnist_upgraded import dataset as mnist_dataset
from official.mnist_upgraded import mnist
...
def train(model, optimizer, dataset, log_interval=None):
  ...
  # can be computed.
      with tf.GradientTape() as tape:
        ...
        if tf.equal(optimizer.iterations % log_interval, 0):
            tf.summary.scalar(name='loss', data=loss_value, step = optimizer.iterations)
            tf.summary.scalar(name='accuracy', data=compute_accuracy(logits, labels), step= optimizer.iterations)
...
def test(model, dataset,step_counter):
  ...
  for (images, labels) in dataset:
    ...
    accuracy.update_state(
        y_pred=tf.argmax(input=logits, axis=1, output_type=tf.int64),
        y_true=tf.cast(labels, tf.int64))
    print('Test set: Average loss: %.4f, Accuracy: %4f%%\n' %
        (avg_loss.result(), 100 * accuracy.result()))

  tf.summary.scalar(name='loss', data=avg_loss.result(), step= step_counter)
  tf.summary.scalar(name='accuracy', data=accuracy.result(), step= step_counter)
...
def run_mnist_eager(flags_obj):
  ...
  optimizer = tf.keras.optimizers.SGD(flags_obj.lr, flags_obj.momentum)
  ...
  with tf.device(device):
    for _ in range(flags_obj.train_epochs):
      ...
      with test_summary_writer.as_default():
        test(model, test_ds,step_counter)
