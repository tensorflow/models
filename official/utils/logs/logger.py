...
class BenchmarkFileLogger(BaseBenchmarkLogger):
    ...
    """
        ...
        with tf.io.gfile.GFile(
                os.path.join(self._logging_dir, BENCHMARK_RUN_LOG_FILE_NAME),
                "w") as f:
            try:
...
class BenchmarkBigQueryLogger(BaseBenchmarkLogger):
    ...
    def __init__(self, bigquery_uploader, bigquery_data_set, bigquery_run_table,
                 bigquery_run_status_table, bigquery_metric_table, run_id):
...
def _gather_run_info(model_name, dataset_name, run_params, test_id):
    ...
    run_info = {
        ...
        "test_id":
            test_id,
        "run_date":
            datetime.datetime.utcnow().strftime(_DATE_TIME_FORMAT_PATTERN)
    }
...
def _process_metric_to_json(name,
                            value,
                            unit=None,
                            global_step=None,
                            extras=None):
