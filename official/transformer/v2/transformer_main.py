  ...
  def train(self):
    ...
    def train_steps(iterator, steps):
      ...
      def _step_fn(inputs):
        ...
        with tf.GradientTape() as tape:
          ...
          scaled_loss = tf.math.divide(
              loss, self.distribution_strategy.num_replicas_in_sync)
    ...
    if params["use_tpu"]:
      ...
      if latest_checkpoint:
        ...
        tf.compat.v1.logging.info("Loaded checkpoint %s", latest_checkpoint)
    ...
    for i in range(1, iterations + 1):
      ...
      if params["use_ctl"]:
        if params["use_tpu"]:
          ...
          tf.compat.v1.logging.info("Train Step: %d/%d / loss = %s",
          ...
          tf.compat.v1.logging.info("Saved checkpoint to %s", checkpoint_name)
        else:
