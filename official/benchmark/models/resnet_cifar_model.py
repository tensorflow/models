...
# ==============================================================================
"""ResNet56 model for Keras adapted from tf_keras.applications.ResNet50.
...
def identity_building_block(input_tensor,
  ...
  x = tf_keras.layers.add([x, input_tensor])
  x = tf_keras.layers.Activation('relu')(x)
  return x
...
def resnet(num_blocks, classes=10, training=None):
  ...
  input_shape = (32, 32, 3)
  img_input = tf_keras.Input(shape=input_shape)

  if tf_keras.backend.image_data_format() == 'channels_first':
    x = tf_keras.layers.Lambda(
        lambda x: tf_keras.backend.permute_dimensions(x, (0, 3, 1, 2)),
        name='transpose')(
            img_input)
    bn_axis = 1
  else:  # channel_last
  ...
  x = tf_keras.layers.ZeroPadding2D(padding=(1, 1), name='conv1_pad')(x)
  x = tf_keras.layers.Conv2D(
      16, (3, 3),
      strides=(1, 1),
      padding='valid',
  ...
  if tf_keras.backend.image_data_format() == 'channels_last':
    ...
    rm_axes = [2, 3]
  x = tf_keras.layers.Lambda(
      lambda x: tf_keras.backend.mean(x, rm_axes), name='reduce_mean')(x)
  ...
  inputs = img_input
  # Create model.
  model = tf_keras.models.Model(inputs, x, name='resnet56')
