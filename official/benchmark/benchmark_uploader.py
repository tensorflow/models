...
class BigQueryUploader(object):
    ...
    def upload_benchmark_run_json(self, dataset_name, table_name, run_id,
                                  run_json):
    ...
    def upload_benchmark_metric_json(self, dataset_name, table_name, run_id,
                                     metric_json_list):
    ...
    def upload_benchmark_run_file(self, dataset_name, table_name, run_id,
                                  run_json_file):
    ...
    """
        with tf.io.gfile.GFile(run_json_file) as f:
            ...
            self.upload_benchmark_run_json(dataset_name, table_name, run_id,
                                           benchmark_json)

    def upload_metric_file(self, dataset_name, table_name, run_id,
                           metric_json_file):
    ...
    """
        with tf.io.gfile.GFile(metric_json_file) as f:
            ...
            self.upload_benchmark_metric_json(dataset_name, table_name, run_id,
                                              metrics)
    ...
    def _upload_json(self, dataset_name, table_name, json_list):
        ...
        if errors:
            tf.logging.error(
                "Failed to upload benchmark info to bigquery: {}".format(
                    errors))
    ...
    def insert_run_status(self, dataset_name, table_name, run_id, run_status):
        ...
        query = ("INSERT {ds}.{tb} "
                 "(run_id, status) "
                 "VALUES('{rid}', '{status}')").format(ds=dataset_name,
                                                       tb=table_name,
                                                       rid=run_id,
                                                       status=run_status)
        try:
    ...
    def update_run_status(self, dataset_name, table_name, run_id, run_status):
        ...
        query = ("UPDATE {ds}.{tb} "
                 "SET status = '{status}' "
                 "WHERE run_id = '{rid}'").format(ds=dataset_name,
                                                  tb=table_name,
                                                  status=run_status,
                                                  rid=run_id)
        try:
