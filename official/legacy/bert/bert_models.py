...
class BertPretrainLossAndMetricLayer(keras.layers.Layer):
  ...
  def _add_metrics(self, lm_output, lm_labels, lm_label_weights,
                   ...
                   next_sentence_loss):
    """Adds metrics."""
    masked_lm_accuracy = keras.metrics.sparse_categorical_accuracy(
        lm_labels, lm_output)
    ...
    if sentence_labels is not None:
      next_sentence_accuracy = keras.metrics.sparse_categorical_accuracy(
  ...
  def call(self,
    ...
    lm_prediction_losses = keras.losses.sparse_categorical_crossentropy(
    ...
    if sentence_labels is not None:
      ...
      sentence_loss = keras.losses.sparse_categorical_crossentropy(
...
def pretrain_model(bert_config,
  ...
  masked_lm_positions = keras.layers.Input(
      ...
      dtype=tf.int32)
  masked_lm_ids = keras.layers.Input(
      shape=(max_predictions_per_seq,), name='masked_lm_ids', dtype=tf.int32)
  masked_lm_weights = keras.layers.Input(
      shape=(max_predictions_per_seq,),
      name='masked_lm_weights',
      dtype=tf.int32)

  if use_next_sentence_label:
    next_sentence_labels = keras.layers.Input(
  ...
  keras_model = keras.Model(inputs=inputs, outputs=output_loss)
...
def classifier_model(bert_config,
  ...
  output = keras.layers.Dropout(rate=bert_config.hidden_dropout_prob)(
      pooled_output)
