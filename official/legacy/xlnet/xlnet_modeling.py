...
def _get_initializer(flags):
  ...
  if flags.init_method == "uniform":
    ...
    initializer = keras.initializers.RandomNormal(stddev=flags.init_std)
  else:
...
def _cache_mem(curr_out, prev_mem, mem_len, reuse_len=None):
  ...
  return keras.backend.stop_gradient(new_mem)
...
class RelativePositionEncoding(keras.layers.Layer):
...
class RelativeAttention(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    ...
    self.attention_probs_dropout = keras.layers.Dropout(
...
class PositionwiseFF(keras.layers.Layer):
...
class EmbeddingLookup(keras.layers.Layer):
...
class RelativeMultiheadAttention(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    ...
    self.attention_dropout = keras.layers.Dropout(rate=self.dropout)
...
class TransformerXLModel(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    ...
    self.h_dropout = keras.layers.Dropout(rate=self.dropout)
    self.g_dropout = keras.layers.Dropout(rate=self.dropout)
    ...
    self.emb_dropout = keras.layers.Dropout(rate=self.dropout)
    ...
    self.output_dropout = keras.layers.Dropout(rate=self.dropout)
...
class PretrainingXLNetModel(keras.Model):
...
class ClassificationXLNetModel(keras.Model):
  ...
  def call(self, features):
    ...
    self.add_loss(keras.backend.mean(per_example_loss))
    return new_mems, logits


class LMLossLayer(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    ...
    if self.use_proj:
      ...
      self.proj_layer_norm = keras.layers.LayerNormalization(
...
class Summarization(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    ...
    if self.use_proj:
      self.proj_layer = keras.layers.Dense(
          ...
          name="summary")
    self.dropout_layer = keras.layers.Dropout(rate=self.dropout_rate)
...
class ClassificationLossLayer(keras.layers.Layer):
...
class QAXLNetModel(keras.Model):
...
class QALossLayer(keras.layers.Layer):
  ...
  def build(self, unused_input_shapes):
    """Implements build() for the layer."""
    self.start_logits_proj_layer = keras.layers.Dense(
        units=1, kernel_initializer=self.initializer, name="start_logits/dense")
    self.end_logits_proj_layer0 = keras.layers.Dense(
        units=self.hidden_size,
        kernel_initializer=self.initializer,
        activation=tf.nn.tanh,
        name="end_logits/dense_0")
    self.end_logits_proj_layer1 = keras.layers.Dense(
        units=1, kernel_initializer=self.initializer, name="end_logits/dense_1")
    self.end_logits_layer_norm = keras.layers.LayerNormalization(
        axis=-1, epsilon=1e-12, name="end_logits/LayerNorm")
    self.answer_class_proj_layer0 = keras.layers.Dense(
        units=self.hidden_size,
        kernel_initializer=self.initializer,
        activation=tf.nn.tanh,
        name="answer_class/dense_0")
    self.answer_class_proj_layer1 = keras.layers.Dense(
        units=1,
        kernel_initializer=self.initializer,
        use_bias=False,
        name="answer_class/dense_1")
    self.ans_feature_dropout = keras.layers.Dropout(rate=self.dropout_rate)
