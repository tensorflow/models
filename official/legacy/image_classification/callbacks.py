...
def get_callbacks(
    ...
    backup_and_restore: bool = False) -> List[keras.callbacks.Callback]:
  """Get all callbacks."""
  ...
  if backup_and_restore:
    ...
    callbacks.append(
        keras.callbacks.experimental.BackupAndRestore(backup_dir))
...
class CustomTensorBoard(keras.callbacks.TensorBoard):
  ...
  def _get_base_optimizer(self) -> keras.optimizers.Optimizer:
...
class AverageModelCheckpoint(keras.callbacks.ModelCheckpoint):
  ...
  Attributes:
    update_weights: If True, assign the moving average weights to the model, and
      ...
      model uses the average weights. See `keras.callbacks.ModelCheckpoint`
      for the other args.
  """
