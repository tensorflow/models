...
def conv2d_block(inputs: tf.Tensor,
  ...
  if activation is not None:
    x = keras.layers.Activation(activation, name=name + '_activation')(x)
  return x
...
def efficientnet(image_input: keras.layers.Input, config: ModelConfig):  # pytype: disable=invalid-annotation  # typed-keras
  """Creates an EfficientNet graph given the model parameters.

  This function is wrapped by the `EfficientNet` class to make a keras.Model.
  ...
  # Build classifier
  x = keras.layers.GlobalAveragePooling2D(name='top_pool')(x)
