...
class Lookahead(keras.optimizers.legacy.Optimizer):
  ...
  ```python
  opt = keras.optimizers.SGD(learning_rate) opt =
  ...
  def __init__(
    ...
    if not isinstance(
        optimizer,
        (keras.optimizers.Optimizer, keras.optimizers.legacy.Optimizer),
    ):
      raise TypeError(
          'optimizer is not an object of keras.optimizers.Optimizer'
      )
...
def build_optimizer(
    optimizer_name: Text,
    base_learning_rate: keras.optimizers.schedules.LearningRateSchedule,
  ...
  Args:
    ...
    params: String -> Any dictionary representing the optimizer params. This
      ...
      `decay`, etc.
    model: The `keras.Model`. This is used for the shadow copy if using
      `ExponentialMovingAverage`.

  Returns:
    A keras.optimizers.legacy.Optimizer.
