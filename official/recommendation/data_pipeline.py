  ...
  """
    ...
    """
        ...
        features = tf.io.parse_single_example(
            serialized_data,
            _get_feature_map(batch_size, is_training=is_training))
        ...
        if is_training:
            valid_point_mask = tf.cast(features[rconst.VALID_POINT_MASK],
                                       tf.bool)
    ...
    def start_construction(self):
        if self._stream_files:
            ...
            template = os.path.join(self.current_data_root,
    ...
    """
        ...
        if self._stream_files:
            ...
            map_fn = functools.partial(self.deserialize,
...
class BaseDataConstructor(threading.Thread):
  ...
  """
    ...
    def __init__(
        ...
        if self._train_pos_users.shape != self._train_pos_items.shape:
            raise ValueError(
                ...
                format(self._train_pos_users.shape,
                       self._train_pos_items.shape))

        (self._train_pos_count,) = self._train_pos_users.shape
        self._elements_in_epoch = (1 +
                                   num_train_negatives) * self._train_pos_count
    ...
    """
        ...
        negative_indices = np.greater_equal(batch_indices,
                                            self._train_pos_count)
        ...
        if pad_length:
            ...
            user_pad = np.arange(pad_length,
                                 dtype=users.dtype) % self._num_users
            item_pad = np.arange(pad_length,
    ...
    def _wait_to_construct_train_epoch(self):
        ...
        while self._train_dataset.buffer_reached() and not self._stop_loop:
            ...
            if count >= 100 and np.log10(count) == np.round(np.log10(count)):
                logging.info(
                    "Waited {} times for training data to be consumed".format(
                        count))
    ...
    def _construct_training_epoch(self):
        ...
        get_pool = (popen_helper.get_fauxpool
    ...
    """
        ...
        users = np.repeat(self._eval_pos_users[low_index:high_index,
                                               np.newaxis],
                          1 + rconst.NUM_EVAL_NEGATIVES,
                          axis=1)
        positive_items = self._eval_pos_items[low_index:high_index, np.newaxis]
        negative_items = (self.lookup_negative_items(
            negative_users=users[:, :-1]).reshape(-1,
                                                  rconst.NUM_EVAL_NEGATIVES))
        ...
        self._eval_dataset.put(
            i, {
                ...
                rconst.DUPLICATE_MASK:
                    np.reshape(duplicate_mask.flatten(),
    ...
    def make_input_fn(self, is_training):
        ...
        if self._fatal_exception is not None:
            raise ValueError(
                "Fatal exception in the data production loop: {}".format(
...
class MaterializedDataConstructor(BaseDataConstructor):
  ...
  """
    ...
    def construct_lookup_variables(self):
        ...
        self._negative_table = np.zeros(shape=(self._num_users,
                                               self._num_items),
        ...
        self._per_user_neg_count = np.zeros(shape=(self._num_users,),
                                            dtype=np.int32)
        ...
        for i in range(self._num_users):
            positives = self._train_pos_items[index_bounds[i]:index_bounds[i +
                                                                           1]]
  ...
  """
    ...
    def construct_lookup_variables(self):
        ...
        inner_bounds = np.argwhere(self._train_pos_users[1:] -
                                   self._train_pos_users[:-1])[:, 0] + 1
        (upper_bound,) = self._train_pos_users.shape
        self.index_bounds = np.array([0] + inner_bounds.tolist() +
                                     [upper_bound])
