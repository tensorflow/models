...
class IncrementEpochCallback(keras.callbacks.Callback):
...
def _get_keras_model(params):
  ...
  valid_pt_mask_input = keras.layers.Input(
      shape=(1,), name=rconst.VALID_POINT_MASK, dtype=tf.bool)

  dup_mask_input = keras.layers.Input(
      shape=(1,), name=rconst.DUPLICATE_MASK, dtype=tf.int32)

  label_input = keras.layers.Input(
  ...
  zeros = keras.layers.Lambda(lambda x: x * 0)(logits)

  softmax_logits = keras.layers.concatenate([zeros, logits], axis=-1)
  ...
  with distribute_utils.get_strategy_scope(strategy):
    ...
    if params["keras_use_ctl"]:
      ...
      if not FLAGS.ml_perf:
        ...
        summary_callback = keras.callbacks.TensorBoard(
