...
class NcfTest(tf.test.TestCase):
    ...
    def test_hit_rate_and_ndcg(self):
        ...
        self.assertAlmostEqual(
            ndcg,
            (1 + math.log(2) / math.log(3) + 2 * math.log(2) / math.log(4)) / 4)

        hr, ndcg = self.get_hit_rate_and_ndcg(predictions,
                                              items,
                                              1,
                                              match_mlperf=True)
        self.assertAlmostEqual(hr, 1 / 4)
        ...
        self.assertAlmostEqual(
            ndcg,
            (1 + math.log(2) / math.log(3) + 2 * math.log(2) / math.log(5)) / 4)
    ...
    def test_end_to_end_estimator(self):
        integration.run_synthetic(ncf_estimator_main.main,
    ...
    def test_end_to_end_keras_no_dist_strat(self):
        integration.run_synthetic(ncf_keras_main.main,
                                  tmp_root=self.get_temp_dir(),
                                  extra_flags=self._BASE_END_TO_END_FLAGS +
    ...
    def test_end_to_end_keras_dist_strat_ctl(self):
        flags = (self._BASE_END_TO_END_FLAGS + ['-num_gpus', '0'] +
                 ['-keras_use_ctl', 'True'])
        integration.run_synthetic(ncf_keras_main.main,
                                  tmp_root=self.get_temp_dir(),
                                  extra_flags=flags)
