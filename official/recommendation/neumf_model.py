...
def neumf_model_fn(features, labels, mode, params):
  ...
  user_input = keras.layers.Input(tensor=users)
  item_input = keras.layers.Input(tensor=items)
...
def construct_model(user_input: tf.Tensor, item_input: tf.Tensor,
                    params: Dict[Text, Any]) -> keras.Model:
  ...
  embedding_user = keras.layers.Embedding(
      ...
      embeddings_regularizer=keras.regularizers.l2(mf_regularization),
      input_length=1,
      name="embedding_user")(
          user_input)
  ...
  # GMF part
  mf_user_latent = keras.layers.Lambda(
      mf_slice_fn, name="embedding_user_mf")(
          embedding_user)
  mf_item_latent = keras.layers.Lambda(
      mf_slice_fn, name="embedding_item_mf")(
          embedding_item)

  # MLP part
  mlp_user_latent = keras.layers.Lambda(
      mlp_slice_fn, name="embedding_user_mlp")(
          embedding_user)
  mlp_item_latent = keras.layers.Lambda(
  ...
  for layer in xrange(1, num_layer):
    model_layer = keras.layers.Dense(
        model_layers[layer],
        kernel_regularizer=keras.regularizers.l2(mlp_reg_layers[layer]),
        activation="relu")
  ...
  model = keras.models.Model([user_input, item_input], logits)
  model.summary()
  sys.stdout.flush()
