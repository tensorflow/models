...
def neumf_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        ...
        return _get_estimator_spec_with_metrics(logits,
                                                softmax_logits,
                                                duplicate_mask,
                                                params["num_neg"],
                                                params["match_mlperf"],
...
def construct_model(user_input, item_input, params):
  ...
  """
    ...
    embedding_item = tf.keras.layers.Embedding(
        ...
        embeddings_regularizer=tf.keras.regularizers.l2(mf_regularization),
        input_length=1,
        name="embedding_item")(item_input)
...
def compute_eval_loss_and_metrics_helper(
  ...
  """
    ...
    def metric_fn(top_k_tensor, ndcg_tensor, weight_tensor):
        return {
            rconst.HR_KEY:
                tf.compat.v1.metrics.mean(top_k_tensor,
                                          weights=weight_tensor,
                                          name=rconst.HR_METRIC_NAME),
            rconst.NDCG_KEY:
                tf.compat.v1.metrics.mean(ndcg_tensor,
                                          weights=weight_tensor,
