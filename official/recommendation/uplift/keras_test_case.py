...
class KerasTestCase(tf.test.TestCase):
  ...
  def toKerasInputs(self, inputs):  # pylint:disable=[invalid-name]
    """Generate tf.keras.Input for inputs.
    ...
    Returns:
      tf.keras.Input representing the input.
    """

    def map_to_keras_input(x):
      if isinstance(x, tf.Tensor):
        return tf.keras.Input(x.shape[1:], dtype=x.dtype)
      ts = tf.type_spec_from_value(x)
      return tf.keras.Input(type_spec=ts)
  ...
  def assertLayerSavable(
    ...
    Args:
      ...
      keras_inputs: if inputs._type_spec won't create a tf.keras.Input.
    ...
    def _make_model(inputs, layer, keras_inputs):
      ...
      return tf.keras.Model(keras_inputs, keras_outputs)
  ...
  def assertModelSavable(
    ...
    tf.keras.models.save_model(model, model_path, save_format=save_format)
  ...
  def assertLayerConfigurable(
      self, layer: tf.keras.layers.Layer, serializable: bool = True, **kwargs
  ):
