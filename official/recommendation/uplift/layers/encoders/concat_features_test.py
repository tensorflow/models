...
class ConcatFeaturesTest(keras_test_case.KerasTestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      {
          ...
          "inputs": {
              "x1": keras.Input(shape=(2, None, 1, 3)),
              "x2": keras.Input(shape=(2, None, 1, None)),
          },
      ...
      },
      {
          ...
          "inputs": {
              "dense": keras.Input(shape=(2, None, 1, 3), batch_size=10),
              "sparse": keras.Input(shape=(2, None, 1, 3), sparse=True),
              "ragged": keras.Input(shape=(2, None, None, 1), ragged=True),
          },
  ...
  def test_layer_correctness_keras_inputs(self, inputs, expected_shape):
    ...
    KerasTensor = keras.Input(shape=(1,)).__class__  # pylint: disable=invalid-name
  ...
  @parameterized.named_parameters(
      ...
      },
      {
          ...
          "inputs": {
              "x1": keras.Input(shape=(2, 3), batch_size=10),
              "x2": keras.Input(shape=(2, 3), batch_size=4),
          },
      },
  )
  ...
  @parameterized.named_parameters(
      ...
      },
      {
          ...
          "inputs": {
              "x1": keras.Input(shape=(2, 3, 4), batch_size=4),
              "x2": keras.Input(shape=(2, 3), batch_size=4),
          },
      },
  )
