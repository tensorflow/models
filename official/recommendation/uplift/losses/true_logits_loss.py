...
class TrueLogitsLoss(keras.__internal__.losses.LossFunctionWrapper):
  ...
  def __init__(
    ...
    Args:
      ...
      reduction: Type of `keras.losses.Reduction` to apply to loss. Default
  ...
  def from_config(cls, config: MutableMapping[str, Any]) -> TrueLogitsLoss:
    config["loss_fn"] = keras.losses.get(config["loss_fn"])
    return cls(**config)
