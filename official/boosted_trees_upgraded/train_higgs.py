...
def _make_csv_serving_input_receiver_fn(column_names, column_defaults):
  ...
  def serving_input_receiver_fn():
    csv = tf.compat.v1.placeholder(dtype=tf.string, shape=[None], name="csv")
    features = dict(zip(column_names, tf.io.decode_csv(records=csv, record_defaults=column_defaults)))
