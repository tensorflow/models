...
class Subtokenizer(object):
    ...
    """
        ...
        if tf.io.gfile.exists(vocab_file):
            tf.compat.v1.logging.info("Vocab file already exists (%s)" %
                                      vocab_file)
        else:
    ...
    def _token_to_subtoken_ids(self, token):
        ...
        ret = _split_token_to_subtokens(_escape_token(token, self.alphabet),
...
def _split_token_to_subtokens(token, subtoken_dict, max_subtoken_length):
    ...
    while start < token_len:
        ...
        for end in xrange(min(token_len, start + max_subtoken_length), start,
                          -1):
