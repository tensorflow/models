...
def get_eval_metrics(logits, labels, params):
    ...
    metrics = {
        "accuracy":
            _convert_to_eval_metric(padded_accuracy)(logits, labels),
        "accuracy_top5":
            _convert_to_eval_metric(padded_accuracy_top5)(logits, labels),
        "accuracy_per_sequence":
            _convert_to_eval_metric(padded_sequence_accuracy)(logits, labels),
        "neg_log_perplexity":
            _convert_to_eval_metric(padded_neg_log_perplexity)
    ...
    if not params["use_tpu"]:
        ...
        metrics.update({
            "approx_bleu_score":
                _convert_to_eval_metric(bleu_score)(logits, labels),
            "rouge_2_fscore":
                _convert_to_eval_metric(rouge_2_fscore)(logits, labels),
            "rouge_L_fscore":
                _convert_to_eval_metric(rouge_l_fscore)(logits, labels),
        })
...
def padded_sequence_accuracy(logits, labels):
    ...
    with tf.variable_scope("padded_sequence_accuracy", values=[logits, labels]):
        ...
        correct_seq = 1.0 - tf.minimum(1.0, tf.reduce_sum(not_correct,
                                                          axis=axis))
