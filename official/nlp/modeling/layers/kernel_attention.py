...
class KernelMask(keras.layers.Layer):
...
def expplus(data_orig,
  ...
  if normalize_data:
    ...
    lengths = tf.reduce_sum(lengths, axis=keras.backend.ndim(data) - 1)
    lengths = tf.expand_dims(lengths, axis=keras.backend.ndim(data) - 1)
  ...
  diag_data = tf.math.reduce_sum(
      diag_data, axis=keras.backend.ndim(data) - 1)
  diag_data = (diag_data / 2.0) * data_normalizer * data_normalizer
  diag_data = tf.expand_dims(diag_data, axis=keras.backend.ndim(data) - 1)
  ...
  diag_omega = tf.math.reduce_sum(
      diag_omega, axis=keras.backend.ndim(projection_matrix) - 1)
...
_CAUSAL_SUPPORT_TRANSFORM_MAP = {
    "elu":
        functools.partial(
            _generalized_kernel,
            f=lambda x: keras.activations.elu(x) + 1,
            h=lambda x: 1),
    "relu":
        functools.partial(
            _generalized_kernel,
            ...
            # a tiny epsilon.
            f=lambda x: keras.activations.relu(x) + 1e-3,
            h=lambda x: 1),
    "square":
...
class KernelAttention(keras.layers.MultiHeadAttention):
  ...
  def _build_from_signature(self, query, value, key=None):
    ...
    if self._begin_kernel > 0:
      ...
      self._dropout_softmax = keras.layers.Dropout(rate=self._dropout)
