...
@gin.configurable
class TransformerScaffold(keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    def get_layer_instance(instance_or_cls, config, default_config):
      if isinstance(instance_or_cls, keras.layers.Layer):
        return instance_or_cls
      elif isinstance(instance_or_cls, dict):
        return get_layer_instance(
            keras.utils.deserialize_keras_object(instance_or_cls),
            config,
            default_config,
        )
      else:
    ...
    self._attention_dropout = keras.layers.Dropout(rate=self._dropout_rate)
    ...
    self._output_dropout = keras.layers.Dropout(rate=self._dropout_rate)
    # Use float32 in layernorm for numeric stability.
