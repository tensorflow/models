...
class RandomFeatureGaussianProcess(keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      use_custom_random_features: (bool) Whether to use custom random
        features implemented using keras.layers.Dense.
    ...
    if self.use_custom_random_features:
      ...
      if self.custom_random_features_initializer is None:
        self.custom_random_features_initializer = (
            keras.initializers.RandomNormal(stddev=1.))
  ...
  def build(self, input_shape):
    ...
    if self.normalize_input:
      self._input_norm_layer = keras.layers.LayerNormalization(
    ...
    self._gp_output_layer = keras.layers.Dense(
        units=self.units,
        use_bias=False,
  ...
  def _make_random_feature_layer(self, name):
    ...
    if not self.use_custom_random_features:
      # Use default RandomFourierFeatures layer from keras.
      return keras.layers.experimental.RandomFourierFeatures(
    ...
    if self.gp_kernel_type.lower() == 'linear':
      custom_random_feature_layer = keras.layers.Lambda(
          lambda x: x, name=name)
    else:
      # Use user-supplied configurations.
      custom_random_feature_layer = keras.layers.Dense(
...
class LaplaceRandomFeatureCovariance(keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.precision_matrix = (
        self.add_weight(
            ...
            dtype=self.dtype,
            initializer=keras.initializers.Identity(self.ridge_penalty),
            trainable=False,
