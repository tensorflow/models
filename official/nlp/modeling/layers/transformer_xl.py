...
class TransformerXLBlock(keras.layers.Layer):
  ...
  **Note: This layer is currently experimental.
  ...
  def build(self, input_shape):
    ...
    self._attention_layer_norm = keras.layers.LayerNormalization(
        ...
        dtype=tf.float32)
    self._inner_dense = keras.layers.EinsumDense(
        "abc,cd->abd",
    ...
    self._inner_activation_layer = keras.layers.Activation(
...
class TransformerXL(keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.output_dropout = keras.layers.Dropout(rate=self._dropout_rate)
