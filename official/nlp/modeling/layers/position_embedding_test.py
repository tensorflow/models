...
class PositionEmbeddingLayerTest(tf.test.TestCase):
  ...
  def test_static_layer_output_shape(self):
    ...
    width = 30
    input_tensor = keras.Input(shape=(sequence_length, width))
  ...
  def test_non_default_axis_static(self):
    ...
    width = 30
    input_tensor = keras.Input(shape=(width, sequence_length, width))
    output_tensor = test_layer(input_tensor)
  ...
  def test_non_default_axis_dynamic(self):
    ...
    # Create a 3-dimensional input (the first dimension is implicit).
    width = 30
    input_tensor = keras.Input(shape=(None, None, width))
    output_tensor = test_layer(input_tensor)
