...
class MobileBertMaskedLMTest(tf.test.TestCase):
  ...
  def test_layer_creation(self):
    ...
    lm_input_tensor = keras.Input(shape=(sequence_length, hidden_size))
    masked_positions = keras.Input(shape=(num_predictions,), dtype=tf.int32)
  ...
  def test_layer_invocation_with_external_logits(self):
    ...
    logit_output = keras.layers.Activation(tf.nn.log_softmax)(logit_output)
    logit_layer.set_weights(test_layer.get_weights())
    model = keras.Model([lm_input_tensor, masked_positions], output)
    logits_model = keras.Model(([lm_input_tensor, masked_positions]),
                                  logit_output)
