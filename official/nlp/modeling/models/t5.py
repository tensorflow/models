...
class Embed(Module):
  ...
  def __init__(self,
    ...
    if embeddings_initializer:
      ...
      self.embed_init = keras.initializers.TruncatedNormal(stddev=1.0)
...
class Linear(Module):
  ...
  def __init__(self,
    ...
    self.w_init = w_init
    if self.use_bias:
      self.b_init = b_init if b_init else keras.initializers.Zeros()
    ...
    with self.name_scope:
      if self.w_init is None:
        ...
        self.w_init = keras.initializers.HeNormal()
...
class Linear3D(Module):
  ...
  def __init__(self,
    ...
    if self.to_3d:
      ...
      bias_rank = 1
    if self.use_bias:
      self.b_init = b_init or keras.initializers.Zeros()
    elif b_init is not None:
      raise ValueError("When not using a bias the b_init must be None.")
...
class T5TransformerParams:
  ...
  weight_initializer: Optional[Initializer] = (keras.initializers.HeNormal())
  bias_initializer: Optional[Initializer] = None
  rescale_query: bool = False
  bidirectional: bool = True
  ffn_activations: Sequence[str] = ("relu",)
