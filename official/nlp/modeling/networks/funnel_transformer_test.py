...
class SingleLayerModel(keras.Model):
...
class FunnelTransformerEncoderTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_network_creation_dense(self, append_dense_inputs):
    ...
    dense_mask = keras.Input(shape=(dense_sequence_length,), dtype=tf.int32)
    dense_type_ids = keras.Input(
  ...
  def test_serialize_deserialize(self):
    ...
    expected_config["inner_activation"] = keras.activations.serialize(
        keras.activations.get(expected_config["inner_activation"]))
    expected_config["initializer"] = keras.initializers.serialize(
        keras.initializers.get(expected_config["initializer"]))
    ...
    _ = keras.models.load_model(model_path)
