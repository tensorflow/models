...
class ValidatedTransformerLayer(layers.Transformer):
  ...
  def get_config(self):
    ...
    config["call_class"] = keras.utils.get_registered_name(self.call_class)
    return config
...
@keras.utils.register_keras_serializable(package="TestLayerOnly")
...
class EncoderScaffoldLayerClassTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_network_creation(self, return_all_layer_outputs):
    ...
    test_network = encoder_scaffold.EncoderScaffold(
        ...
        pooler_layer_initializer=keras.initializers.TruncatedNormal(
            stddev=0.02),
    ...
    self.assertIsInstance(test_network.pooler_layer, keras.layers.Dense)
  ...
  def test_network_invocation(self):
    ...
    model = keras.Model([word_ids, mask, type_ids], outputs)
...
class Embeddings(keras.Model):
  ...
  def __init__(self, vocab_size, hidden_size):
    ...
    self.inputs = [
        ...
        keras.layers.Input(shape=(None,), dtype=tf.int32, name="input_mask")
    ]
...
class EncoderScaffoldEmbeddingNetworkTest(tf.test.TestCase):
  ...
  def test_network_invocation(self):
    ...
    # Create a model based off of this network:
    model = keras.Model([word_ids, mask], [data, pooled])
  ...
  def test_serialize_deserialize(self):
    ...
    network = keras.Model([word_ids, mask],
    ...
    new_model = keras.Model([word_ids, mask], [data, pooled])
...
class EncoderScaffoldHiddenInstanceTest(
  ...
  def test_network_invocation(self):
    ...
    # Create a model based off of this network:
    model = keras.Model([word_ids, mask, type_ids], [data, pooled])
  ...
  def test_hidden_cls_list(self):
    ...
    model_a = keras.Model([word_ids, mask], [data_a, pooled_a])
    # Create model based off of network b:
    data_b, pooled_b = test_network_b([word_ids, mask])
    model_b = keras.Model([word_ids, mask], [data_b, pooled_b])
    # Create model based off of network b:
    data_c, pooled_c = test_network_c([word_ids, mask])
    model_c = keras.Model([word_ids, mask], [data_c, pooled_c])
  ...
  def test_serialize_deserialize(self, use_hidden_cls_instance):
    ...
    new_model = keras.Model([word_ids, mask, type_ids], [data, pooled])
