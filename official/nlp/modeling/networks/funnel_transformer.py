...
def _get_policy_dtype():
  try:
    return keras.mixed_precision.global_policy().compute_dtype or tf.float32
...
class FunnelTransformerEncoder(keras.layers.Layer):
  ...
  def __init__(
    ...
    # TODO(crickwu): explore keras.layers.serialize method.
    if pool_type == _MAX:
      pool_cls = keras.layers.MaxPooling1D
    elif pool_type == _AVG:
      pool_cls = keras.layers.AveragePooling1D
