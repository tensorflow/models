...
class SpanLabelingTest(tf.test.TestCase):
  ...
  def test_network_invocation(self):
    ...
    model = keras.Model(sequence_data, outputs)
  ...
  def test_network_invocation_with_internal_logit_output(self):
    ...
    model = keras.Model(sequence_data, output)
    ...
    input_tensor = keras.Input(expected_output_shape[1:])
    output_tensor = keras.layers.Activation(tf.nn.log_softmax)(input_tensor)
    softmax_model = keras.Model(input_tensor, output_tensor)
  ...
  def test_network_invocation_with_external_logit_output(self):
    ...
    model = keras.Model(sequence_data, output)
    logit_model = keras.Model(sequence_data, logit_output)
...
class XLNetSpanLabelingTest(tf.test.TestCase):
  ...
  def test_subclass_invocation(self):
    ...
    sequence_data = keras.Input(shape=(seq_length, hidden_size),
                                   dtype=tf.float32)
    class_index = keras.Input(shape=(), dtype=tf.uint8)
    paragraph_mask = keras.Input(shape=(seq_length), dtype=tf.float32)
    start_positions = keras.Input(shape=(), dtype=tf.int32)
