...
class PackedSequenceEmbedding(keras.Model):
  """An embedding network supporting packed sequences and position ids.
  ...
  def __init__(self,
    ...
    if use_position_id:
      position_ids = keras.layers.Input(
    ...
    embeddings = keras.layers.LayerNormalization(
        name='embeddings/layer_norm', axis=-1, epsilon=1e-12, dtype=tf.float32)(
            embeddings)
    embeddings = keras.layers.Dropout(
...
@keras.utils.register_keras_serializable(package='Text')
class PackedSequenceMask(keras.layers.Layer):
...
@keras.utils.register_keras_serializable(package='Text')
class PositionEmbeddingWithSubSeqMask(keras.layers.Layer):
