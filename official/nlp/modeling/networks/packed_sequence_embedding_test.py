...
class PackedSequenceEmbeddingTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_network_creation(self, use_position_id, pack_multiple_sequences,
    ...
    input_word_ids = keras.Input(shape=(seq_length,), dtype=tf.int32)
    input_mask = keras.Input(shape=(seq_length,), dtype=tf.int32)
    input_type_ids = keras.Input(shape=(seq_length,), dtype=tf.int32)
    network_inputs = {
    ...
    if use_position_id:
      network_inputs['position_ids'] = keras.Input(
          shape=(seq_length,), dtype=tf.int32)

    embedding, mask = test_object(network_inputs)

    # Create a model based off of this network:
    model = keras.Model(network_inputs, [embedding, mask])
