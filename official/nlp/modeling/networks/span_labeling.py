...
class SpanLabeling(keras.Model):
  ...
  *Note* that the network is constructed by
  ...
  def __init__(self,
    ...
    sequence_data = keras.layers.Input(
        shape=(None, input_width), name='sequence_data', dtype=tf.float32)

    intermediate_logits = keras.layers.Dense(
    ...
    start_predictions = keras.layers.Activation(tf.nn.log_softmax)(
        start_logits)
    end_predictions = keras.layers.Activation(tf.nn.log_softmax)(end_logits)
...
class XLNetSpanLabeling(keras.layers.Layer):
  ...
  **Note: `compute_with_beam_search` will not work with the Functional API
  ...
  def __init__(self,
    ...
    self.start_logits_dense = keras.layers.Dense(
        units=1,
        kernel_initializer=tf_utils.clone_initializer(initializer),
        name='predictions/transform/start_logits')

    self.end_logits_inner_dense = keras.layers.Dense(
        units=input_width,
        kernel_initializer=tf_utils.clone_initializer(initializer),
        activation=activation,
    ...
    self.end_logits_output_dense = keras.layers.Dense(
        units=1,
        kernel_initializer=tf_utils.clone_initializer(initializer),
        name='predictions/transform/end_logits/output')

    self.answer_logits_inner = keras.layers.Dense(
        units=input_width,
        kernel_initializer=tf_utils.clone_initializer(initializer),
        activation=activation,
        name='predictions/transform/answer_logits/inner')
    self.answer_logits_dropout = keras.layers.Dropout(rate=dropout_rate)
    self.answer_logits_output = keras.layers.Dense(
        units=1,
        kernel_initializer=tf_utils.clone_initializer(initializer),
        use_bias=False,
