...
def create_preprocessing(*,
                         ...
                         default_seq_length: int) -> keras.Model:
  ...
  Returns:
    A keras.Model object with several attached subobjects, suitable for
  ...
  sentences = keras.layers.Input(shape=(), dtype=tf.string, name="sentences")
  ...
  preprocessing = keras.Model(sentences, model_inputs)
  ...
  # in their own right.
  preprocessing.tokenize = keras.Model(sentences, tokens)
  ...
  preprocessing.tokenize.get_special_tokens_dict.__call__ = tf.function(
      ...
      input_signature=[])
  if tokenize_with_offsets:
    preprocessing.tokenize_with_offsets = keras.Model(
  ...
  # preprocessing.bert_pack_inputs = keras.Model(tokens, model_inputs)
