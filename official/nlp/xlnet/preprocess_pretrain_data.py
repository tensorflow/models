...
def create_data(_):
    ...
    record_prefix = "record_info-{}-{}-{}".format(FLAGS.split, FLAGS.task,
                                                  FLAGS.pass_id)
    record_name = format_filename(prefix=record_prefix,
                                  bsz_per_host=FLAGS.bsz_per_host,
                                  seq_len=FLAGS.seq_len,
                                  mask_alpha=FLAGS.mask_alpha,
                                  mask_beta=FLAGS.mask_beta,
...
def _split_a_and_b(data, sent_ids, begin_idx, tot_len, extend_target=False):
    ...
    if begin_idx + tot_len >= data_len:
        tf.logging.info(
            ...
            "begin_idx %d + tot_len %d >= data_len %d", begin_idx, tot_len,
            data_len)
        return None
    ...
    if extend_target:
        if a_end >= data_len or b_end >= data_len:
            tf.logging.info(
                "[_split_a_and_b] returns None: "
                "a_end %d or b_end %d >= data_len %d", a_end, b_end, data_len)
            return None
...
def _is_start_piece(piece):
    ...
    if (piece.startswith("▁") or piece.startswith("<") or
            piece in special_pieces):
        return True
    else:
        return False
...
def create_tfrecords(save_dir, basename, data, bsz_per_host, seq_len, bi_data,
                     sp):
    ...
    if bi_data:
        ...
        sent_ids = np.concatenate([fwd_sent_ids, bwd_sent_ids],
                                  1).reshape(bsz_per_host, -1)
    else:
        data, sent_ids = batchify(data, bsz_per_host, sent_ids)

    tf.logging.info("Raw data shape %s.", data.shape)

    file_name = format_filename(prefix=basename,
    ...
    while i + seq_len <= data_len:
        ...
        for idx in range(bsz_per_host):
            ...
            results = _split_a_and_b(data[idx],
                                     sent_ids[idx],
            ...
            mask_0 = _sample_mask(sp,
                                  inp,
                                  reverse=reverse,
                                  goal_num_predict=num_predict_0)
            mask_1 = _sample_mask(sp,
                                  np.concatenate([
                                      a_data, sep_array, b_data, sep_array,
                                      cls_array
                                  ]),
                                  reverse=reverse,
...
def get_input_fn(tfrecord_dir,
    ...
    record_glob_base = format_filename(prefix="record_info-{}-*".format(split),
                                       bsz_per_host=bsz_per_host,
                                       seq_len=seq_len,
                                       bi_data=bi_data,
                                       suffix="json",
        ...
        for record_info_path in record_paths:
            if num_passes is not None:
                ...
                if len(fields) == 5 and pass_id >= num_passes:
                    tf.logging.info("Skip pass %d: %s", pass_id,
                                    record_info_name)
                    continue
            ...
            with tf.gfile.Open(record_info_path, "r") as fp:
                ...
                if num_passes is not None:
                    ...
                    cur_record_info["num_batch"] += int(info["num_batch"] *
                                                        ratio)
                    cur_record_info["filenames"] += info[
                        "filenames"][:eff_num_passes]
                else:
    ...
    def input_fn(params):
        ...
        dataset = get_dataset(params=params,
                              num_hosts=num_hosts,
                              num_core_per_host=num_core_per_host,
                              split=split,
