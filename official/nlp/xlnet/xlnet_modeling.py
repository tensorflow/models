...
class PositionwiseFF(tf.keras.layers.Layer):
    ...
    def build(self, unused_input_shapes):
        ...
        self.inner_projection_layer = (tf.keras.layers.Dense(
            units=self.d_inner,
    ...
    def build(self, unused_input_shapes):
        ...
        for i in range(self.n_layer):
            self.rel_multihead_layers.append(
                RelativeMultiheadAttention(d_model=self.d_model,
            ...
            self.h_positionwise_ffn_layers.append(
                PositionwiseFF(d_model=self.d_model,
...
class ClassificationXLNetModel(tf.keras.Model):
  ...
  """
    ...
    def call(self, features):
        ...
        per_example_loss, logits = self.cl_loss_layer(hidden=summary,
                                                      labels=label)
...
class LMLossLayer(tf.keras.layers.Layer):
    ...
    def call(self, hidden, target, lookup_table, target_mask):
        ...
        if self.tie_weight:
            logits = tf.einsum('ibd,nd->ibn', hidden,
                               lookup_table) + self.softmax_b
        else:
            logits = tf.einsum('ibd,nd->ibn', hidden,
                               self.softmax_w) + self.softmax_b

        if self.use_tpu:
            one_hot_target = tf.one_hot(target,
                                        self.n_token,
                                        dtype=logits.dtype)
            loss = -tf.reduce_sum(
        ...
        total_loss = tf.reduce_sum(
            loss * target_mask) / tf.reduce_sum(target_mask)
...
class Summarization(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.summary_type == 'last':
            ...
            raise ValueError('Invalid summary type provided: %s' %
                             self.summary_type)
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            end_logits = tf.reshape(
                end_logits, [seq_len, -1, self.start_n_top, self.d_model])
