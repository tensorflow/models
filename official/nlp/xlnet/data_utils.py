...
def create_squad_dataset(file_path, seq_length, batch_size, is_training):
    ...
    if is_training:
        ...
        name_to_features["is_impossible"] = tf.io.FixedLenFeature([],
                                                                  tf.float32)
...
def get_squad_input_data(batch_size, seq_len, q_len, strategy, is_training,
    ...
    if is_training:
        input_glob = os.path.join(
            file_path, "spiece.model.*.slen-{}.qlen-{}.train.tf_record".format(
                seq_len, q_len))
...
def _idx_pair_to_mask(beg_indices, end_indices, inputs, tgt_len, num_predict):
    ...
    non_func_mask = tf.logical_and(tf.not_equal(inputs, SEP_ID),
    ...
    def parser(record):
        ...
        if reuse_len > 0:
            ...
            perm_mask_1, target_mask_1, input_k_1, input_q_1 = _local_perm(
                inputs[reuse_len:], is_masked[reuse_len:], perm_size,
                non_reuse_len, leak_ratio)
