...
def _save_checkpoint(checkpoint, model_dir, checkpoint_prefix):
  ...
  tf.logging.info('Saving model as TF checkpoint: %s', saved_path)
  return
  ...
  with tf.device(get_primary_cpu_task(use_remote_tpu)):
    ...
    with strategy.scope():
      ...
      def _run_evaluation(current_training_step, test_iterator):
        ...
        tf.logging.info('Step: [%d] Validation metric = %f',
                        current_training_step, metric.result())
      ...
      if latest_checkpoint_file:
        ...
        tf.logging.info('Loading from checkpoint file completed')
      ...
      while current_step < total_training_steps:
        ...
        if train_metric:
          ...
          tf.logging.info('Train Step: %d/%d  / loss = %s', current_step,
        ...
        if current_step % steps_per_epoch == 0:
          ...
          if eval_input_fn:
            tf.logging.info('Running evaluation after step: %s.', current_step)
      ...
      if eval_input_fn:
        tf.logging.info('Running final evaluation after training is complete.')
        _run_evaluation(current_step,
                        strategy.make_dataset_iterator(eval_input_fn()))
