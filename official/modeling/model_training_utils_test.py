...
def create_model_fn(input_shape, num_classes, use_float16=False):
    ...
    def _model_fn():
        ...
        model.add_metric(tf.reduce_mean(input_layer),
                         name='mean_input',
                         aggregation='mean')
        model.optimizer = tf.keras.optimizers.SGD(learning_rate=0.1,
                                                  momentum=0.9)
        if use_float16:
            model.optimizer = (
...
class ModelTrainingUtilsTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_train_eager_single_step(self, distribution):
        ...
        if isinstance(distribution, tf.distribute.experimental.TPUStrategy):
            with self.assertRaises(ValueError):
                self.run_training(distribution,
                                  model_dir,
                                  steps_per_loop=1,
                                  run_eagerly=True)
        else:
    ...
    def test_train_eager_mixed_precision(self, distribution):
        ...
        self._model_fn = create_model_fn(input_shape=[128],
                                         num_classes=3,
                                         use_float16=True)
        self.run_training(distribution,
                          model_dir,
    ...
    def test_train_check_artifacts(self, distribution):
        ...
        self.assertNotEmpty(
            tf.io.gfile.glob(os.path.join(model_dir, 'ctl_step_*')))
        self.assertNotEmpty(
            tf.io.gfile.glob(
        ...
        self.assertTrue(
            check_eventfile_for_keyword(
                'loss', os.path.join(model_dir, 'summaries/train')))
        self.assertTrue(
            check_eventfile_for_keyword(
                'accuracy', os.path.join(model_dir, 'summaries/train')))
        self.assertTrue(
            check_eventfile_for_keyword(
                'mean_input', os.path.join(model_dir, 'summaries/train')))
        self.assertTrue(
            check_eventfile_for_keyword(
                'accuracy', os.path.join(model_dir, 'summaries/eval')))
        self.assertTrue(
            check_eventfile_for_keyword(
                'mean_input', os.path.join(model_dir, 'summaries/eval')))
