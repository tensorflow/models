...
class DistributedExecutor(object):
  ...
  """
    ...
    """
        _replicated_step = self._create_replicated_step(strategy, model,
                                                        loss_fn, optimizer,
                                                        metric)
      ...
      """
            ...
            losses = tf.nest.map_structure(
                lambda x: strategy.reduce(
                    tf.distribute.ReduceOp.MEAN, x, axis=None),
                per_replica_losses)
            return losses
    ...
    def _create_test_step(self, strategy, model, metric):
        ...
        def test_step(iterator):
            ...
            if not metric:
                logging.info('Skip test_step because metric is None (%s)',
                             metric)
                return None, None
            ...
            return strategy.experimental_run_v2(_test_step_fn,
                                                args=(next(iterator),))
    ...
    """
        ...
        while current_step < total_steps:
            ...
            train_loss = train_step(
                train_iterator, tf.convert_to_tensor(num_steps, dtype=tf.int32))
...
class ExecutorBuilder(object):
  ...
  """
    ...
    """
        ...
        return class_ctor(strategy=self._strategy,
                          params=params,
                          model_fn=model_fn,
                          loss_fn=loss_fn,
                          **kwargs)
