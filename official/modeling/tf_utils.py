...
@deprecation.deprecated(
    None,
    "keras.layers.Layer supports multiple positional args and kwargs as "
...
def get_activation(identifier, use_keras_layer=False, **kwargs):
  ...
  names and callable identifiers, always fallback to keras.activations.get.
...
def serialize_layer(layer, use_legacy_format=False):
  if (
      "use_legacy_format"
      in inspect.getfullargspec(keras.layers.serialize).args
  ):
...
def serialize_initializer(initializer, use_legacy_format=False):
  if (
      "use_legacy_format"
      in inspect.getfullargspec(keras.initializers.serialize).args
  ):
    return keras.initializers.serialize(
...
def serialize_regularizer(regularizer, use_legacy_format=False):
  if (
      "use_legacy_format"
      in inspect.getfullargspec(keras.regularizers.serialize).args
  ):
    return keras.regularizers.serialize(
        regularizer, use_legacy_format=use_legacy_format
    )
  else:
    return keras.regularizers.serialize(regularizer)


def serialize_constraint(constraint, use_legacy_format=False):
  if (
      "use_legacy_format"
      in inspect.getfullargspec(keras.constraints.serialize).args
  ):
    return keras.constraints.serialize(
        constraint, use_legacy_format=use_legacy_format
    )
  else:
    return keras.constraints.serialize(constraint)
