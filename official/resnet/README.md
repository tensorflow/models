...
### Pre-trained model
You can download a 190 MB pre-trained version of ResNet-50 achieving 75.3% top-1 single-crop accuracy here: [resnet50_2017_11_30.tar.gz](http://download.tensorflow.org/models/official/resnet50_2017_11_30.tar.gz). Simply download and uncompress the file, and point the model to the extracted directory using the `--model_dir` flag. This model was trained on a GPU and uses the "channels_first" (also referred to as NCHW) [data format](https://www.tensorflow.org/performance/performance_guide#data_formats).
