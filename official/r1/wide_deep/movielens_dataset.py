...
_FEATURE_MAP = {
    movielens.USER_COLUMN:
        tf.compat.v1.FixedLenFeature([1], dtype=tf.int64),
    movielens.ITEM_COLUMN:
        tf.compat.v1.FixedLenFeature([1], dtype=tf.int64),
    movielens.TIMESTAMP_COLUMN:
        tf.compat.v1.FixedLenFeature([1], dtype=tf.int64),
    movielens.GENRE_COLUMN:
        tf.compat.v1.FixedLenFeature([movielens.N_GENRE], dtype=tf.int64),
    movielens.RATING_COLUMN:
        tf.compat.v1.FixedLenFeature([1], dtype=tf.float32),
}
...
def _df_to_input_fn(df, name, dataset, data_dir, batch_size, repeat, shuffle):
    ...
    file_io.write_to_buffer(dataframe=df,
                            ...
                            columns=list(_FEATURE_MAP.keys()),
...
def _check_buffers(data_dir, dataset):
    ...
    return all([
        tf.gfile.Stat(
            _buffer_path(data_dir, dataset,
                         "train")).length == _BUFFER_SIZE[dataset]["train"],
        tf.gfile.Stat(_buffer_path(
            data_dir, dataset, "eval")).length == _BUFFER_SIZE[dataset]["eval"],
    ])
