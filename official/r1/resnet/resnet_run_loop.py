...
def get_synth_input_fn(height,
  ...
  """
    ...
    def input_fn(is_training, data_dir, batch_size, *args, **kwargs):
        ...
        inputs = tf.random.truncated_normal([batch_size] +
...
def image_bytes_serving_input_fn(image_shape, dtype=tf.float32):
    ...
    image_bytes_list = tf.compat.v1.placeholder(shape=[None],
                                                dtype=tf.string,
                                                name='input_tensor')
...
def override_flags_and_set_envars_for_gpu_thread_pool(flags_obj):
  ...
  """
    ...
    flags_obj.datasets_num_private_threads = (cpu_count -
...
################################################################################
def learning_rate_with_decay(batch_size,
                             batch_denom,
                             num_images,
                             boundary_epochs,
                             decay_rates,
  ...
  """
    ...
    def learning_rate_fn(global_step):
        ...
        lr = tf.compat.v1.train.piecewise_constant(global_step, boundaries,
                                                   vals)
        if warmup:
    ...
    """
        ...
        wrate = (plr * tf.cast(global_step, tf.float32) /
                 tf.cast(w_steps, tf.float32))
