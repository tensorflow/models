...
def translate_and_compute_bleu(estimator, subtokenizer, bleu_source, bleu_ref):
    ...
    translate.translate_file(estimator,
                             subtokenizer,
                             bleu_source,
  ...
  **Step vs. Epoch vs. Iteration**
  ...
  """
    ...
    if evaluate_bleu and schedule_manager.use_tpu:
        raise ValueError(
            "BLEU score can not be computed when training with a TPU, "
    ...
    for i in xrange(schedule_manager.train_eval_iterations):
        ...
        if evaluate_bleu:
            ...
            summary = tf.Summary(value=[
                tf.Summary.Value(tag="bleu/uncased",
                                 simple_value=uncased_score),
                tf.Summary.Value(tag="bleu/cased", simple_value=cased_score),
            ])
            bleu_writer.add_summary(summary, global_step)
            bleu_writer.flush()
            benchmark_logger.log_metric("bleu_uncased",
                                        uncased_score,
                                        global_step=global_step)
            benchmark_logger.log_metric("bleu_cased",
                                        cased_score,
                                        global_step=global_step)
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  """
    ...
    run_config = tf.estimator.tpu.RunConfig(cluster=tpu_cluster_resolver,
                                            model_dir=flags_obj.model_dir,
