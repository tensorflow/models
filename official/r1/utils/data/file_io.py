            ...
            for i in self.temp_buffers:
                if tf.io.gfile.exists(i):
                    ...
                    tf.compat.v1.logging.info(
                        "Buffer file {} removed".format(i))
        except Exception as e:
            tf.compat.v1.logging.error(
                "Failed to cleanup buffer files: {}".format(e))
...
def iter_shard_dataframe(df, rows_per_core=1000):
  ...
  """
    ...
    for i in range(num_blocks):
        ...
        yield [
            df_shard[boundaries[j]:boundaries[j + 1]] for j in range(num_cores)
        ]
...
def _shard_dict_to_examples(shard_dict):
    ...
    for column, values in shard_dict.items():
        ...
        if values.dtype.kind == "i":
            feature_map = lambda x: tf.train.Feature(int64_list=tf.train.
                                                     Int64List(value=x))
        elif values.dtype.kind == "f":
            feature_map = lambda x: tf.train.Feature(float_list=tf.train.
                                                     FloatList(value=x))
        else:
...
def _serialize_shards(df_shards, columns, pool, writer):
  ...
  """
    ...
    map_inputs = [{c: np.stack(shard[c].values, axis=0)
                   for c in columns}
  ...
  """
    if (tf.io.gfile.exists(buffer_path) and
        ...
        tf.compat.v1.logging.warning(
            ...
            "rebuilding buffer.".format(buffer_path, actual_size,
                                        expected_size))
    ...
    tf.compat.v1.logging.info(
        "Constructing TFRecordDataset buffer: {}".format(buffer_path))
    ...
    try:
        with tf.io.TFRecordWriter(buffer_path) as writer:
            for df_shards in iter_shard_dataframe(df=dataframe,
                ...
                tf.compat.v1.logging.info("{}/{} examples written.".format(
