...
TEST_CASES = [
    dict(embedding_dim=256,
         vocab_size=1000,
         sequence_length=64,
         batch_size=32,
         seed=54131),
    dict(embedding_dim=8,
         vocab_size=15,
         sequence_length=12,
         batch_size=256,
         seed=536413),
    dict(embedding_dim=2048,
         vocab_size=512,
         sequence_length=50,
         batch_size=8,
         seed=35124)
...
class TPUBaseTester(tf.test.TestCase):
    ...
    def construct_embedding_and_values(self, embedding_dim, vocab_size,
        ...
        embedding_table = tf.convert_to_tensor(value=embeddings,
                                               dtype=tf.float32)

        tokens = np.random.randint(low=1,
                                   high=vocab_size - 1,
    ...
    def _test_embedding(self, embedding_dim, vocab_size, sequence_length,
                        batch_size, seed):
        ...
        with self.test_session():
            ...
            embedding = (
                tf.nn.embedding_lookup(params=embedding_table, ids=values) *
