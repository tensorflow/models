  ...
  """
    ...
    source_columns = [
        tf.feature_column.numeric_column(
            ...
            default_value=0.0) for feature_name in feature_names
    ]
    bucketized_columns = [
        tf.feature_column.bucketized_column(source_columns[i],
                                            boundaries=get_bucket_boundaries(
                                                features_np_list[i]))
...
def make_eval_inputs_from_np_arrays(features_np, label_np):
    ...
    def input_fn():
        ...
        return tf.data.Dataset.zip((
            ...
            tf.data.Dataset.from_tensor_slices(label_np),
        )).batch(1000)
...
def train_boosted_trees(flags_obj):
  ...
  """
    ...
    train_data, eval_data = read_higgs_data(flags_obj.data_dir,
                                            flags_obj.train_start,
                                            flags_obj.train_count,
                                            flags_obj.eval_start,
                                            flags_obj.eval_count)
    ...
    eval_input_fn = make_eval_inputs_from_np_arrays(features_np=eval_data[:,
                                                                          1:],
                                                    label_np=eval_data[:, 0:1])
    ...
    benchmark_logger.log_run_info(model_name="boosted_trees",
                                  dataset_name="higgs",
...
def define_train_higgs_flags():
    ...
    flags_core.define_base(clean=False,
    ...
    flags.DEFINE_integer("n_trees",
                         default=100,
                         help=help_wrap("Number of trees to build."))
    flags.DEFINE_integer("max_depth",
                         default=6,
