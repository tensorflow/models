...
class MixupAndCutmix:
    ...
    def __init__(self,
        ...
        Args:
          ...
          label_smoothing (float, optional): Constant for label smoothing. Defaults
            to 0.1.
          one_hot (bool): Whether input truth labels are one-hot encoded or not.
            Defaults to true.
          num_classes (int, optional): Number of classes. Defaults to 1001.
        """
        if not one_hot and num_classes is None:
            raise ValueError("`num_classes` argument should not be `None`"
                             "if `one_hot` argument is `False`.")
    ...
    def distort(self, images: tf.Tensor,
        ...
        # Cast input to tf.float32 type in case of mixed-precision training
        ...
        def augment_a():
            _images, _labels = self._update_labels(*tf.cond(
                tf.less(
                    ...
                    images, labels)))
            return tf.cast(_images, images_dtype), _labels

        def augment_b():
            return tf.cast(images, images_dtype), self._smooth_labels(labels)
