...
class BASNetExportTest(tf.test.TestCase, parameterized.TestCase):

  def _get_basnet_module(self):
    params = exp_factory.get_exp_config('basnet_duts')
    #params.task.model.backbone.resnet.model_id = 18
    basnet_module = basnet.BASNetModule(
        params, batch_size=1, input_image_size=[256, 256])
  ...
  """
  def test_export(self, input_type='image_bytes'):
    #tmp_dir = self.get_temp_dir()
    tmp_dir = '/home/ghpark/serving_test/'

    module = self._get_basnet_module()
    ...
    #basnet_fn = imported.signatures['serving_default']

    #images = self._get_dummy_input(input_type)
    images = np.array(Image.open('img1_test.jpg'))
    processed_images = tf.image.resize(images, [256, 256])
    processed_images = tf.reshape(processed_images, [1, 256, 256, 3])
    ...
    processed_images = tf.nest.map_structure(
        ...
        tf.map_fn(
            ...
            fn_output_signature=tf.TensorSpec(
                shape=[256, 256, 3], dtype=tf.float32)))
    """
    expected_output = model(processed_images, training=False)
    out = tf.cast(expected_output['ref']*255, dtype=tf.uint8)
    out = out.numpy()
    print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
    print(out)
    print(out.shape)
...
tmp_dir = '/home/serving_test/'

module = self._get_basnet_module()
model = module.build_model()

self._export_from_module(module, 'image_bytes', tmp_dir)

imported = tf.saved_model.load(tmp_dir)
basnet_fn = imported.signatures['serving_default']

images = Image.open('img1_test.jpg')
