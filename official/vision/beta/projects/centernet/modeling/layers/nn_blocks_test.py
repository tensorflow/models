...
class HourglassBlockPyTorch(tf.keras.layers.Layer):
  ...
  def make_layer(self, k, inp_dim, out_dim, modules, **kwargs):
    layers = [official_nn_blocks.ResidualBlock(out_dim, 1, use_projection=True, **kwargs)]
    for _ in range(1, modules):
      layers.append(official_nn_blocks.ResidualBlock(out_dim, 1, **kwargs))
    return tf.keras.Sequential(layers)

  def make_layer_revr(self, k, inp_dim, out_dim, modules, **kwargs):
    layers = []
    for _ in range(modules - 1):
      layers.append(official_nn_blocks.ResidualBlock(inp_dim, 1, **kwargs))  # inp_dim is not a bug
    layers.append(official_nn_blocks.ResidualBlock(out_dim, 1, use_projection=True, **kwargs))
    return tf.keras.Sequential(layers)
