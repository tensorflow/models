...
class InputUtilsTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def testResizeCropFilter(self, default_width, default_height, num_boxes, target_width, target_height):
      image = tf.convert_to_tensor(np.random.rand(default_width, default_height, 3))
      boxes = tf.convert_to_tensor(np.random.rand(num_boxes, 4))
      resized_image, resized_boxes = preprocessing_ops.resize_crop_filter(image, boxes, default_width, default_height, target_width, target_height)
      resized_image_shape = tf.shape(resized_image)
      resized_boxes_shape = tf.shape(resized_boxes)
      self.assertAllEqual([default_height, default_width, 3], resized_image_shape.numpy())
  ...
  def testTranslateBoxes(self, num_boxes, translate_x, translate_y):
    boxes = tf.convert_to_tensor(np.random.rand(num_boxes, 4))
    translated_boxes = preprocessing_ops.translate_boxes(boxes, translate_x, translate_y)
  ...
  def testTranslateImage(self, image_height, image_width, translate_x, translate_y):
    image = tf.convert_to_tensor(np.random.rand(image_height, image_width, 4))
    translated_image = preprocessing_ops.translate_image(image, translate_x, translate_y)
    translated_image_shape = tf.shape(translated_image)
    self.assertAllEqual([image_height, image_width, 4], translated_image_shape.numpy())
  ...
  def testPadMaxInstances(self, input_shape, instances, pad_axis):
    ...
    output = preprocessing_ops.pad_max_instances(np.ones(input_shape), instances, pad_axis = pad_axis)
