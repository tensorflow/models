...
from official.vision.beta.projects.yolo import configs as yolo_config
...
def create_yolo_export_module(params: cfg.ExperimentConfig,
                              input_type: str,
                              batch_size: int,
                              input_image_size: List[int],
                              num_channels: int = 3):
  ...
  def preprocess_fn(inputs):
    ...
    def preprocess_image_fn(inputs):
      ...
      image = image / 255.
      (image, image_info) = yolo_model_fn.letterbox(image, input_image_size,
  ...
  def inference_steps(inputs, model):
    ...
    detection['bbox'] = yolo_model_fn.undo_info(detection['bbox'],
  ...
  if isinstance(params.task,
    ...
    export_module = create_classification_export_module(
        params, input_type, batch_size, input_image_size, num_channels)
                  ...
                  yolo_config.yolo.YoloTask):
    export_module = create_yolo_export_module(
      params, input_type, batch_size, input_image_size, num_channels)
  else:
