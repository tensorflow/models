...
class COCOWrapper(coco.COCO):
  ...
  """
    ...
    """
        if ((annotation_file and gt_dataset) or ((not annotation_file) and
                                                 (not gt_dataset))):
            raise ValueError(
    ...
    """
        ...
        for ann in predictions:
            x1, x2, y1, y2 = [
                ann['bbox'][0], ann['bbox'][0] + ann['bbox'][2], ann['bbox'][1],
        ...
        for j in range(batch_size):
            ...
            for k in range(num_instances):
                ...
                if 'masks' in groundtruths:
                    mask = Image.open(
                        six.StringIO(groundtruths['masks'][i][j, k]))
...
def scan_and_generator_annotation_file(file_pattern, num_samples, include_mask,
                                       annotation_file):
    """Scans and generate the COCO-style annotation JSON file given a dataset."""
    groundtruth_generator = COCOGroundtruthGenerator(file_pattern, num_samples,
                                                     include_mask)
