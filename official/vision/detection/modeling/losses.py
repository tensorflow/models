            ...
            for level in levels:
                score_losses.append(
                    self._rpn_score_loss(
                        score_outputs[level],
                        labels[level],
                        normalizer=tf.cast(tf.shape(score_outputs[level])[0] *
...
class RpnBoxLoss(object):
    ...
    """
        with tf.name_scope('rpn_loss'):
            ...
            for level in levels:
                box_losses.append(
                    self._rpn_box_loss(box_outputs[level],
                                       labels[level],
                                       delta=self._delta))
...
class FastrcnnClassLoss(object):
    ...
    """
        with tf.name_scope('fast_rcnn_loss'):
            ...
            return self._fast_rcnn_class_loss(class_outputs,
        ...
        with tf.name_scope('fast_rcnn_loss'):
            ...
            box_indices = tf.reshape(
                class_targets + tf.tile(
                    ...
                    [1, num_rois]) +
                tf.tile(tf.expand_dims(tf.range(num_rois) * num_classes, 0),
                        [batch_size, 1]), [-1])

            box_outputs = tf.matmul(
                tf.one_hot(box_indices,
                           batch_size * num_rois * num_classes,
                           dtype=box_outputs.dtype),
                tf.reshape(box_outputs, [-1, 4]))
            box_outputs = tf.reshape(box_outputs, [batch_size, -1, 4])

            return self._fast_rcnn_box_loss(box_outputs,
                                            box_targets,
                                            class_targets,
                                            delta=self._delta)
...
class RetinanetClassLoss(object):
    ...
    """
        ...
        for level in cls_outputs.keys():
            cls_losses.append(
                self.class_loss(cls_outputs[level], labels[level],
                                num_positives_sum))
        # Sums per level losses to total loss.
...
class RetinanetBoxLoss(object):
    ...
    def box_loss(self, box_outputs, box_targets, num_positives):
        ...
        box_loss = self._huber_loss(box_targets,
                                    box_outputs,
                                    sample_weight=mask)
        box_loss /= normalizer
        return box_loss
