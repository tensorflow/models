...
class Resnet(object):
    ...
    """
        ...
        if use_projection:
            ...
            shortcut = self.conv2d_fixed_padding(inputs=inputs,
                                                 filters=filters,
                                                 kernel_size=1,
                                                 strides=strides)
    ...
    """
        ...
        inputs = self.conv2d_fixed_padding(inputs=inputs,
                                           ...
                                           kernel_size=1,
                                           strides=1)
        inputs = self._batch_norm_relu(relu=False,
                                       init_zero=True)(inputs,
                                                       is_training=is_training)
        inputs = self._dropblock(inputs, is_training=is_training)
    ...
    """
        ...
        def model(inputs, is_training=None):
            ...
            c2 = self.block_group(inputs=inputs,
                                  filters=64,
                                  block_fn=block_fn,
                                  blocks=layers[0],
                                  strides=1,
