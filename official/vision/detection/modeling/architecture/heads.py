...
class RpnHead(tf.keras.layers.Layer):
    ...
    def _shared_rpn_heads(self, features, anchors_per_location, level,
        ...
        if self._use_batch_norm:
            ...
            features = self._batch_norm_relus[level](features,
    ...
    def __call__(self, fpn_features, is_training=None):
        ...
        with backend.get_graph().as_default(), tf.name_scope('retinanet'):
            for level in range(self._min_level, self._max_level + 1):
                ...
                class_outputs[level] = self.class_net(features,
                                                      level,
                                                      is_training=is_training)
                box_outputs[level] = self.box_net(features,
                                                  level,
                                                  is_training=is_training)
        return class_outputs, box_outputs
...
class ShapemaskPriorHead(object):
            ...
            if self._shape_prior_path:
                if self._use_category_for_mask:
                    ...
                    class_masks = np.zeros(
                        (max_class_id, self._num_clusters, self._mask_crop_size,
    ...
    """
        ...
        with tf.name_scope('masknet'):
            ...
            for feat_level in range(self._min_mask_level,
                ...
                mask_feats[feat_level] = tf.keras.layers.Conv2D(
                    ...
                    kernel_initializer=tf.keras.initializers.RandomNormal(
                        stddev=0.01),
                    padding='same',
                    name='mask-downsample')(fpn_features[feat_level])
            ...
            for feat_level in range(self._min_mask_level + 1,
                                    self._max_mask_level + 1):
                features.append(
                    tf.image.pad_to_bounding_box(mask_feats[feat_level], 0, 0,
                                                 self._max_feature_size,
                                                 self._max_feature_size))
    ...
    """
        if self._use_category_for_mask:
            object_class_priors = tf.gather(self.class_priors,
                                            detection_prior_classes)
        else:
            num_batch_instances = shape_weights.get_shape()[0]
            object_class_priors = tf.tile(tf.expand_dims(self.class_priors, 0),
                                          [num_batch_instances, 1, 1, 1])

        vector_class_priors = tf.reshape(object_class_priors, [
            -1, self._num_clusters, self._mask_crop_size * self._mask_crop_size
        ])
        detection_priors = tf.matmul(tf.expand_dims(shape_weights, 1),
                                     vector_class_priors)[:, 0, :]
...
class ShapemaskCoarsemaskHead(object):
    ...
    def __init__(self, num_classes, num_downsample_channels, mask_crop_size,
                 use_category_for_mask, num_convs):
    ...
    """
        with backend.get_graph().as_default(), tf.name_scope(
            ...
            point_samp_prob_thr = tf.cast(point_samp_prob_thr,
                                          class_probs.dtype)
            class_probs = tf.where(tf.greater(class_probs, point_samp_prob_thr),
                                   class_probs, tf.zeros_like(class_probs))
