...
def filter_boxes(boxes, scores, image_shape, min_size_threshold):
  ...
  """
    ...
    with tf.name_scope('filter_boxes'):
        ...
        min_size = tf.cast(tf.math.maximum(min_size_threshold, 1.0),
                           dtype=boxes.dtype)

        filtered_size_mask = tf.math.logical_and(tf.math.greater(h, min_size),
                                                 tf.math.greater(w, min_size))
        filtered_center_mask = tf.logical_and(
            tf.math.logical_and(tf.math.greater(yc, 0.0),
                                tf.math.less(yc, height)),
            tf.math.logical_and(tf.math.greater(xc, 0.0),
                                tf.math.less(xc, width)))
...
def top_k_boxes(boxes, scores, k):
  ...
  """
    with tf.name_scope('top_k_boxes'):
        ...
        if batch_size == 1:
            selected_boxes = tf.squeeze(tf.gather(boxes, top_k_indices, axis=1),
                                        axis=1)
        else:
