...
def policy_vtest():
    ...
    # sub-policy that will be applied sequentially on the image.
    policy = [
        [('Rotate_BBox', 0.8, 10), ('Equalize', 1.0, 10)],
    ]
    return policy
...
def policy_v2():
    ...
    policy = [
        ...
        [('Color', 0.8, 4), ('TranslateY_BBox', 1.0, 6), ('Rotate_BBox', 0.6,
                                                          6)],
        [('Rotate_BBox', 0.6, 10), ('BBox_Cutout', 1.0, 4), ('Cutout', 0.2,
                                                             8)],
        [('Rotate_BBox', 0.0, 0), ('Equalize', 0.6, 6), ('ShearY_BBox', 0.6,
                                                         8)],
...
def random_shift_bbox(image,
    ...
    minval_y = clip_y(min_y - tf.cast(
        pixel_scaling * tf.cast(bbox_height, tf.float32) / 2.0, tf.int32))
    maxval_y = clip_y(min_y + tf.cast(
        pixel_scaling * tf.cast(bbox_height, tf.float32) / 2.0, tf.int32))
    minval_x = clip_x(min_x - tf.cast(
        pixel_scaling * tf.cast(bbox_width, tf.float32) / 2.0, tf.int32))
    maxval_x = clip_x(min_x + tf.cast(
        pixel_scaling * tf.cast(bbox_width, tf.float32) / 2.0, tf.int32))
    ...
    if new_min_bbox_coords is None:
        ...
        unclipped_new_min_y, unclipped_new_min_x = (clip_y(
            new_min_bbox_coords[0]), clip_x(new_min_bbox_coords[1]))
    ...
    def mask_and_add_image(min_y_, min_x_, max_y_, max_x_, mask,
        ...
        """Applies mask to bbox region in image then adds content_tensor to
        it."""
        mask = tf.pad(
            mask, [[min_y_, (image_height - 1) - max_y_],
                   [min_x_, (image_width - 1) - max_x_], [0, 0]],
            constant_values=1)
        content_tensor = tf.pad(
            content_tensor, [[min_y_, (image_height - 1) - max_y_],
                             [min_x_, (image_width - 1) - max_x_], [0, 0]],
            constant_values=0)
...
def _check_bbox_area(min_y, min_x, max_y, max_x, delta=0.05):
    ...
    if tf.equal(height, 0.0):
        max_y = tf.maximum(max_y, 0.0 + delta)
        min_y = tf.minimum(min_y, 1.0 - delta)

    if tf.equal(width, 0.0):
        max_x = tf.maximum(max_x, 0.0 + delta)
        min_x = tf.minimum(min_x, 1.0 - delta)
...
def _concat_bbox(bbox, bboxes):
    """Helper function that concates bbox to bboxes along the first
    dimension."""
    ...
    bboxes = bbox if tf.equal(bboxes_sum_check, -4.0) else tf.concat(
        [bboxes, bbox], 0)
    return bboxes
...
def _apply_bbox_augmentation_wrapper(image, bbox, new_bboxes, prob,
    ...
    if func_changes_bbox:
        augmented_image = augmentation_func(
            image, bbox, *args) if should_apply_op else (image, bbox)
    else:
        augmented_image = _apply_bbox_augmentation(
            image, bbox, augmentation_func,
            *args) if should_apply_op else image
...
def _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
    ...
    # will be thrown away.
    bboxes = tf.constant(_INVALID_BOX) if tf.equal(tf.size(bboxes),
                                                   0) else bboxes
...
def _apply_multi_bbox_augmentation_wrapper(image, bboxes, prob, aug_func,
    ...
    if tf.equal(num_bboxes, 0):
        ...
        return _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
                                              func_changes_bbox, *args)
...
def _rotate_bbox(bbox, image_height, image_width, degrees):
    ...
    rotation_matrix = tf.stack([[tf.cos(radians),
                                 tf.sin(radians)],
                                [-tf.sin(radians),
                                 tf.cos(radians)]])
    new_coords = tf.cast(
    ...
    min_y = -(tf.cast(tf.reduce_max(new_coords[0, :]), tf.float32) /
              image_height - 0.5)
    min_x = tf.cast(tf.reduce_min(new_coords[1, :]),
                    tf.float32) / image_width + 0.5
    max_y = -(tf.cast(tf.reduce_min(new_coords[0, :]), tf.float32) /
              image_height - 0.5)
...
def rotate_with_bboxes(image, bboxes, degrees, replace):
    ...
    image_width = tf.shape(image)[1]
    # pylint:disable=g-long-lambda
    wrapped_rotate_bbox = lambda bbox: _rotate_bbox(bbox, image_height,
                                                    image_width, degrees)
    # pylint:enable=g-long-lambda
...
def equalize(image):
    ...
    def scale_channel(im, c):
        ...
        if tf.equal(step, 0):
            ...
            return tf.cast(
                tf.gather(tf.clip_by_value(lut, 0, 255), im), tf.uint8)

    # Assumes RGB for now.  Scales each channel independently
    # and then stacks the result.
    scales = [scale_channel(image, c) for c in range(3)]
    image = tf.stack(scales, 2)
    return image
...
def unwrap(image, replace):
    ...
    alpha_channel = flattened_image[:, 3:4]
...
def bbox_cutout(image, bboxes, pad_fraction, replace_with_mean):
    ...
    if tf.not_equal(tf.size(bboxes), 0):
...
def _bbox_cutout_level_to_arg(level, hparams):
    cutout_pad_fraction = (
        level / _MAX_LEVEL) * hparams.cutout_max_pad_fraction
...
def _get_func_args(func):
    return [
        arg.strip() for arg in str(inspect.signature(func))[1:-1].split(',')
    ]


def _parse_policy_info(name, prob, level, replace_value, augmentation_hparams):
    """Return the function that corresponds to `name` and update `level`
    param."""
    ...
    if 'replace' in arg_names:
        ...
        assert 'replace' == arg_names[-1]
        args = tuple(list(args) + [replace_value])

    # Add bboxes as the second positional argument for the function if it does
    # not already exist.
    if 'bboxes' not in arg_names:
...
def select_and_apply_random_policy(policies, image, bboxes):
    ...
    for (i, policy) in enumerate(policies):
        if tf.equal(i, policy_to_select):
