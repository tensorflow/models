...
class SelfGating(keras.layers.Layer):
  ...
  def build(self, input_shape):
    self._spatial_temporal_average = keras.layers.GlobalAveragePooling3D()

    # No BN and activation after conv.
    self._transformer_w = keras.layers.Conv3D(
...
@keras.utils.register_keras_serializable(package='Vision')
class BottleneckBlock3D(keras.layers.Layer):
  ...
  def build(self, input_shape):
    self._shortcut_maxpool = keras.layers.MaxPool3D(
        pool_size=[1, 1, 1],
        strides=[
            self._temporal_strides, self._spatial_strides, self._spatial_strides
        ])

    self._shortcut_conv = keras.layers.Conv3D(
    ...
    self._temporal_conv = keras.layers.Conv3D(
        filters=self._filters,
    ...
    self._spatial_conv = keras.layers.Conv3D(
        filters=self._filters,
        kernel_size=[1, 3, 3],
    ...
    self._expand_conv = keras.layers.Conv3D(
        filters=4 * self._filters,
        kernel_size=[1, 1, 1],
        strides=[1, 1, 1],
        padding='same',
