...
def define_flags():
    ...
    flags.DEFINE_boolean(name='enable_eager',
                         default=True,
                         help='Enable eager?')
    flags.DEFINE_boolean(name='train',
                         default=True,
...
def make_prediction(checkpoint_dir, length, context, idx2char, char2idx):
  ...
  """
    prediction_model = build_model(vocab_size=len(idx2char),
                                   batch_size=1,
                                   stateful=True)
...
def run(flags_obj):
  ...
  """
    ...
    stats = {}
    if flags_obj.train:
        history, callbacks = train_model(flags_obj,
                                         dataset,
                                         len(idx2char),
                                         strategy,
    ...
    if flags_obj.predict_context:
        ...
        print(
            make_prediction(flags_obj.model_dir, flags_obj.predict_length,
                            flags_obj.predict_context, idx2char, char2idx))
