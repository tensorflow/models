...
@keras.utils.register_keras_serializable(package='Vision')
class SegmentationHeadQuantized(keras.layers.Layer):
...
class BatchNormalizationWrapper(keras.layers.Wrapper):
      ...
      if self._config_dict['use_depthwise_convolution']:
        self._convs.append(
            helper.DepthwiseConv2DQuantized(
                ...
                padding='same',
                use_bias=False,
                depthwise_initializer=keras.initializers.RandomNormal(
                    stddev=0.01),
                depthwise_regularizer=self._config_dict['kernel_regularizer'],
                depth_multiplier=1,
                activation=helper.NoOpActivation()))
