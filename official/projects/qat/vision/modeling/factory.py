...
def build_qat_classification_model(
  ...
  Args:
    ...
    l2_regularizer: keras.regularizers.Regularizer object. Default to None.
...
def build_qat_retinanet(
    model: keras.Model, quantization: common.Quantization,
    model_config: configs.retinanet.RetinaNet) -> keras.Model:
  ...
  with tfmot.quantization.keras.quantize_scope(scope_dict):
    ...
    if quantization.quantize_detection_decoder:
      ...
      decoder = keras.models.clone_model(
          decoder,
...
def build_qat_segmentation_model(
    model: keras.Model, quantization: common.Quantization,
    input_specs: keras.layers.InputSpec) -> keras.Model:
  ...
  # Apply QAT to backbone (a keras.Model) first.
  with tfmot.quantization.keras.quantize_scope(scope_dict):
    annotated_backbone = tfmot.quantization.keras.quantize_annotate_model(
        model.backbone)
    optimized_backbone = tfmot.quantization.keras.quantize_apply(
