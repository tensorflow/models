...
class RoformerEncoder(keras.Model):
  ...
  *Note* that the network is constructed by
  ...
  def __init__(
      ...
      type_vocab_size=16,
      inner_dim=3072,
      inner_activation=lambda x: keras.activations.gelu(x, approximate=True),
      output_dropout=0.1,
    ...
    embeddings = keras.layers.Add()([word_embeddings, type_embeddings])

    embedding_norm_layer = keras.layers.LayerNormalization(
        name='embeddings/layer_norm', axis=-1, epsilon=1e-12, dtype=tf.float32)

    embeddings = embedding_norm_layer(embeddings)
    embeddings = (keras.layers.Dropout(rate=output_dropout)(embeddings))

    # We project the 'embedding' output to 'hidden_size' if it is not already
    # 'hidden_size'.
    if embedding_width != hidden_size:
      embedding_projection = keras.layers.EinsumDense(
          '...x,xy->...y',
