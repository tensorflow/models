...
def build_maskrcnn(input_specs: keras.layers.InputSpec,
                   model_config: deep_mask_head_rcnn_config.DeepMaskHeadRCNN,
                   l2_regularizer: keras.regularizers.Regularizer = None):  # pytype: disable=annotation-type-mismatch  # typed-keras
...
class DeepMaskHeadRCNNTask(maskrcnn.MaskRCNNTask):
  ...
  def build_model(self):
    ...
    dummy_images = keras.Input(self.task_config.model.input_size)
    dummy_image_shape = keras.layers.Input([2])
