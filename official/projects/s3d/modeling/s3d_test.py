...
class S3dTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_build(self, num_frames, height, width, first_temporal_kernel_size):
    ...
    input_specs = keras.layers.InputSpec(shape=input_shape)
    network = s3d.S3D(
  ...
  def test_serialize_deserialize(self):
    ...
    kwargs = dict(
        input_specs=keras.layers.InputSpec(shape=(5, 64, 224, 224, 3)),
        ...
        norm_momentum=0.999,
        norm_epsilon=0.001,
        temporal_conv_initializer=keras.initializers.TruncatedNormal(
