...
def inception_v1_stem_cells(
  ...
  if use_sync_bn:
    batch_norm = keras.layers.experimental.SyncBatchNormalization
  else:
...
def _construct_branch_3_layers(
    ...
    pool_type: Text,
    batch_norm_layer: keras.layers.Layer,
...
class InceptionV1CellLayer(keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self._branch_0_layers = [
        keras.layers.Conv3D(**branch_params[0][0]),
        self._norm(**branch_params[0][1]),
        keras.layers.Activation('relu', **branch_params[0][2]),
    ]

    self._branch_1_layers = [
        keras.layers.Conv3D(**branch_params[1][0]),
        self._norm(**branch_params[1][1]),
        keras.layers.Activation('relu', **branch_params[1][2]),
        self._parameterized_conv_layer(**branch_params[1][3]),
    ]

    self._branch_2_layers = [
        keras.layers.Conv3D(**branch_params[2][0]),
        self._norm(**branch_params[2][1]),
        keras.layers.Activation('relu', **branch_params[2][2]),
        self._parameterized_conv_layer(**branch_params[2][3])
    ]

    if self._swap_pool_and_1x1x1:
      self._branch_3_layers = [
          keras.layers.Conv3D(**branch_params[3][0]),
          self._norm(**branch_params[3][1]),
          keras.layers.Activation('relu', **branch_params[3][2]),
          keras.layers.MaxPool3D(**branch_params[3][3]),
      ]
    else:
      self._branch_3_layers = [
          keras.layers.MaxPool3D(**branch_params[3][3]),
