...
class ConstCLPretrainTask(video_ssl_pretrain.VideoSSLPretrainTask):
  ...
  def build_model(self):
    ...
    input_specs_dict = {
        'image':
            keras.layers.InputSpec(shape=[None] + common_input_shape),
        'instances_position':
            keras.layers.InputSpec(
                shape=[None, num_frames, num_instances, 4]),
        'instances_mask':
            keras.layers.InputSpec(shape=[None, num_frames, num_instances]),
    }
  ...
  def build_metrics(self, training=True):
    ...
    metrics = [
        keras.metrics.Mean(name='regularization_loss'),

        keras.metrics.Mean(name='global_loss/loss'),
        keras.metrics.Mean(name='global_loss/contrastive_accuracy'),
        keras.metrics.Mean(name='global_loss/contrastive_entropy'),

        keras.metrics.Mean(name='local_loss/loss'),
        keras.metrics.Mean(name='local_loss/positive_similarity_mean'),
        keras.metrics.Mean(name='local_loss/positive_similarity_max'),
        keras.metrics.Mean(name='local_loss/positive_similarity_min'),
        keras.metrics.Mean(name='local_loss/negative_similarity_mean'),
        keras.metrics.Mean(name='local_loss/negative_similarity_max'),
        keras.metrics.Mean(name='local_loss/negative_similarity_min'),
    ]
    return metrics
