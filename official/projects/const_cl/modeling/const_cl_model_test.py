...
class ConstClModelTest(tf.test.TestCase):
  ...
  def test_build_const_cl_pretrain_model(self):
    ...
    images_input_specs = keras.layers.InputSpec(
        shape=[None, 16, 224, 224, 4])
    boxes_input_specs = keras.layers.InputSpec(shape=[None, 16, 8, 4])
    masks_input_specs = keras.layers.InputSpec(shape=[None, 16, 8])
