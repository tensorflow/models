...
@gin.configurable
class UniversalDetector(keras.layers.Layer):
  """Univeral Detector."""
  loss_items = ("loss_pq", "loss_inst_dist", "loss_para", "loss_mask_id",
                "loss_segmentation_word")

  def __init__(self,
               backbone_fn: keras.layers.Layer = get_max_deep_lab_backbone,
...
class PseudoLayer(keras.layers.Layer):
...
def _get_embed_head(
    dimension=256,
    norm_fn=keras.layers.BatchNormalization
) -> Tuple[keras.Sequential, keras.Sequential]:
  """Projection layers to get instance & grouping features."""
  instance_head = keras.Sequential([
      keras.layers.Dense(dimension, use_bias=False),
      norm_fn(),
      keras.layers.ReLU(),
  ])
  grouping_head = keras.Sequential([
      keras.layers.Dense(dimension, use_bias=False),
      norm_fn(),
      keras.layers.ReLU(),
  ])
...
def _get_para_head(
    dimension=128,
    num_layer=3,
    extra_fc=False) -> Tuple[keras.layers.Layer, keras.layers.Layer]:
  ...
  dense = keras.layers.Dense(dimension)
