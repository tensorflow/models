...
@keras.utils.register_keras_serializable(package='Vision')
class BASNetEncoder(keras.Model):
  """BASNet encoder."""
...
def build_basnet_encoder(
    ...
    model_config,
    l2_regularizer: keras.regularizers.Regularizer = None) -> keras.Model:  # pytype: disable=annotation-type-mismatch  # typed-keras
...
@keras.utils.register_keras_serializable(package='Vision')
class BASNetDecoder(keras.layers.Layer):
  """BASNet decoder."""
  ...
  def __init__(self,
    ...
    self._sigmoid = keras.layers.Activation(activation='sigmoid')
  ...
  def build(self, input_shape):
    ...
    for spec in BASNET_BRIDGE_SPECS:
      ...
      self._out_convs.append(conv_op(
          ...
          padding='same',
          **conv_kwargs))
      self._out_usmps.append(keras.layers.UpSampling2D(
          size=spec[6],
