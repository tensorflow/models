...
def decode_object_seq_to_bbox(logits,
  ...
  mask_s1 = [0.] * (pix2seq_cfg.BASE_VOCAB_SHIFT)  # reserved.
  mask_s2 = [1.] * (coord_vocab_shift - pix2seq_cfg.BASE_VOCAB_SHIFT)  # labels.
  ...
  pred_num = tf.reduce_sum(
          tf.cast(
              tf.math.greater(
                  tf.math.reduce_max(pred_num, axis=-1), 0),
              tf.int32),
          axis=-1)
...
def inject_noise_bbox(boxes, classes, max_instances_per_image):
  ...
  if num_instances < max_instances_per_image:
    ...
    boxes, classes = augment_bbox(
        boxes, classes, 0., n_noise_bbox)
...
def reorder_object_instances(boxes, classes, order):
  ...
  if order == 'random':
    ...
    areas = tf.cast(tf.reduce_prod(boxes[:, 1, :] - boxes[:, 0, :], axis=1),
...
def preserve_reserved_tokens(points, points_orig):
  ...
  return replace_reserved_tokens(points, points_orig, dict(zip(pix2seq_cfg.FLOATS,
                                                               pix2seq_cfg.FLOATS)))
