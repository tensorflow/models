...
class Parser(parser.Parser):
  ...
  def _parse_train_data(self, data):
    ...
    boxes = preprocess_ops.resize_and_crop_boxes(boxes, image_info[2, :],
                                                 ...
                                                 image_info[3, :])
    scale = tf.cast(
        tf.concat([self._output_size,self._output_size],-1),
        boxes.dtype)
    boxes = boxes / scale
    ...
    boxes, classes = utils.inject_noise_bbox(boxes, classes, self._max_num_boxes)
    ...
    prompt_seq = utils.build_prompt_seq_from_task_id(
        pix2seq_cfg.OD_ID, response_seq)  # (1)
    ...
    target_seq = utils.pad_to_max_len(target_seq, self._max_num_boxes*5 + 1, -1)

    input_seq, target_seq = input_seq[..., :-1], target_seq[..., 1:]
    token_weights = utils.pad_to_max_len(token_weights, self._max_num_boxes*5, -1)

    # Assign lower weights for ending/padding tokens.
    token_weights = tf.where(
        target_seq == pix2seq_cfg.PADDING_TOKEN,
        tf.zeros_like(token_weights) + self._eos_token_weight, token_weights)
  ...
  def build_response_seq_from_bbox(self, 
                                 bbox,
                                 label,
                                 coord_vocab_shift,
                                 quantization_bins,
    ...
    # Get token weights.
    is_real = tf.cast(tf.not_equal(new_label, pix2seq_cfg.FAKE_CLASS_TOKEN), tf.float32)
