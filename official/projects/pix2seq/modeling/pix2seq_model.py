...
def add_seq_pos_emb(self, pos_encoding, max_seq_len, dim,
  ...
  if pos_encoding == 'learned':
    self.seq_pos_emb = self.add_weight(
        shape=(max_seq_len+1, dim), initializer=initializer,
...
class Pix2Seq(tf.keras.Model):
  ...
  def _generate_image_mask(self, inputs: tf.Tensor,
    ...
    mask = tf.expand_dims(
        tf.cast(tf.not_equal(tf.reduce_sum(inputs, axis=-1), 0.3), inputs.dtype),
        axis=-1)
    mask = tf.image.resize(
        mask, target_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    return mask

  def call(self, inputs: tf.Tensor, targets: tf.Tensor, training: bool = None) -> List[Any]:
...
class Pix2SeqTransformer(tf.keras.layers.Layer):
  ...
  def __init__(self, max_seq_len, vocab_size, hidden_size,
               pos_encoding='learned', num_encoder_layers=6,
               num_decoder_layers=6, dropout_rate=0.1,
               attention_dropout_rate=0.0, norm_first=True,
               shared_embedding = True, output_bias = True,
               num_heads = 8, **kwargs):
    super().__init__(**kwargs)
    self._dropout_rate = dropout_rate
    self._attention_dropout_rate = attention_dropout_rate
    ...
    add_seq_pos_emb(self, self._pos_encoding, 
                    self._max_seq_len, self._hidden_size)
    add_vocab_token_emb(self, self._vocab_size, self._hidden_size,
                        self._shared_embedding, self._output_bias)
  ...
  def infer(self, inputs: tf.Tensor, max_seq_len=None,
            temperature=1.0, top_k=0, top_p=0.4, sampling_callback=None):
    ...
    def loop_body(step, caches, tokens, logits, is_prompt=False):
      ...
      next_logits = tf.matmul(  # only take the last for sampling next token.
          decoded, outp_embedding, transpose_b=True)[:,-1]
      if self._output_bias:
