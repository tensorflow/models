...
class Pix2SeqTask(base_task.Task):
  ...
  def build_model(self):
    ...
    model = pix2seq_model.Pix2Seq(backbone,
  ...
  def build_losses(self, outputs, labels, aux_losses=None):
    ...
    loss = tf.keras.losses.CategoricalCrossentropy(
        from_logits=True,reduction=tf.keras.losses.Reduction.NONE)(targets, outputs)
  ...
  def validation_step(self, inputs, model, metrics=None):
    ...
    outputs = utils.decode_object_seq_to_bbox(
        logits, tokens, self._task_config.quantization_bins,
    ...
    scale = (
        ...
        utils.tf_float32(labels['image_info'][:, 1:2, :]))
