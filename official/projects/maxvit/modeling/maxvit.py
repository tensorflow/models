...
class MaxViTBlock(keras.layers.Layer):
  ...
  def build(self, input_shape: tf.TensorShape) -> None:
    ...
    self._block_attn_layer_norm = keras.layers.LayerNormalization(
        axis=-1,
    ...
    self._grid_attn_layer_norm = keras.layers.LayerNormalization(
        axis=-1,
        epsilon=self._ln_epsilon,
        dtype=self._ln_dtype,
    ...
    self._block_ffn_layer_norm = keras.layers.LayerNormalization(
        axis=-1,
        epsilon=self._ln_epsilon,
        dtype=self._ln_dtype,
        name='ffn_layer_norm',
    ...
    self._grid_ffn_layer_norm = keras.layers.LayerNormalization(
        axis=-1,
        epsilon=self._ln_epsilon,
        dtype=self._ln_dtype,
  ...
  def build(self, input_shape: tf.TensorShape) -> None:
    if self._norm_type == 'layer_norm':
      bn_class = functools.partial(
          keras.layers.LayerNormalization, epsilon=self._ln_epsilon
      )
    ...
    self._stem = keras.Sequential(layers=stem_layers, name='stem')
    ...
    if self._representation_size and self._representation_size > 0:
      ...
      if self._add_gap_layer_norm:
        self._final_layer_norm = keras.layers.LayerNormalization(
...
def build_maxvit(
  ...
  dummy_inputs = keras.Input(input_specs.shape[1:])
