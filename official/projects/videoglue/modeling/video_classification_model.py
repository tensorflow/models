...
class MultiHeadVideoClassificationModel(keras.Model):
  ...
  def __init__(
    ...
    if classifier_type == 'linear':
      pool_or_flatten_op = keras.layers.GlobalAveragePooling3D()
    ...
    if classifier_type == 'linear':
      for nc in num_classes:
        x = keras.layers.Dropout(dropout_rate)(input_embeddings)
