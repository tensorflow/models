...
class AddTemporalPositionEmbs(keras.layers.Layer):
  """Adds learned temporal positional embeddings to the video features."""

  def __init__(self,
               posemb_init: Optional[keras.initializers.Initializer] = None,
               **kwargs):
...
class MLP(keras.layers.Layer):
  ...
  def __init__(
    ...
    Args:
      ...
      kernel_regularizer: keras.regularizers.Regularizer object.
      bias_regularizer: keras.regularizers.Regularizer object.
    ...
    # Projection head
    self._layers.append(keras.layers.Dense(num_output_channels))
...
class AttentionPoolerClassificationHead(keras.layers.Layer):
  ...
  def __init__(
      ...
      kernel_regularizer: Optional[
          keras.regularizers.Regularizer] = keras.regularizers.L2(1.5e-5),
      bias_regularizer: Optional[keras.regularizers.Regularizer] = None,
    ...
    if self._add_temporal_pos_embed:
      self._pos_embed = AddTemporalPositionEmbs(
          posemb_init=keras.initializers.RandomNormal(stddev=0.02),
          name='posembed_final_learnt',
      )

    self._pooler_attention_layer_norm = keras.layers.LayerNormalization(
        name='pooler_attention_layer_norm',
        axis=-1,
        epsilon=1e-6,
        dtype=tf.float32)

    self._pooler_attention_layer = keras.layers.MultiHeadAttention(
        num_heads=num_heads,
    ...
    self._classifier = keras.layers.Dense(
        num_classes,
