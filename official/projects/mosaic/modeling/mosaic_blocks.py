...
class MultiKernelGroupConvBlock(keras.layers.Layer):
  ...
  def __init__(
    ...
    if self._use_sync_bn:
      self._bn_op = keras.layers.experimental.SyncBatchNormalization
    else:
  ...
  def build(self, input_shape: tf.TensorShape) -> None:
    ...
    if self._use_depthwise_convolution:
      ...
      for i, conv_kernel_size in enumerate(self._kernel_sizes):
        ...
        self._conv_branches.append(conv_branch)
    self._concat_groups = keras.layers.Concatenate(
...
@keras.utils.register_keras_serializable(package='Vision')
class MosaicEncoderBlock(keras.layers.Layer):
  ...
  def __init__(
    ...
    if dropout_rate:
      self._encoder_end_dropout_layer = keras.layers.Dropout(
          rate=dropout_rate)
    else:
  ...
  def build(
    ...
    for pyramid_pool_bin_num in self._pyramid_pool_bin_nums:
      if pyramid_pool_bin_num == 1:
        ...
        global_projection = keras.layers.Conv2D(
        ...
        bin_pool_level = keras.layers.AveragePooling2D(
    ...
    self._upsample = keras.layers.Resizing(
        height,
        width,
        interpolation=self._interpolation,
        crop_to_aspect_ratio=False)
    self._concat_layer = keras.layers.Concatenate(axis=self._channel_axis)
...
@keras.utils.register_keras_serializable(package='Vision')
class DecoderSumMergeBlock(keras.layers.Layer):
  ...
  def __init__(
    ...
    self._bn_axis = (
        -1
        if keras.backend.image_data_format() == 'channels_last' else 1)
    self._channel_axis = (
        -1
        if keras.backend.image_data_format() == 'channels_last' else 1)
    self._add_layer = keras.layers.Add()
  ...
  def build(
    ...
    if low_res_channels != self._decoder_projected_depth:
      low_res_feature_conv = keras.layers.Conv2D(
    ...
    if high_res_channels != self._decoder_projected_depth:
      high_res_feature_conv = keras.layers.Conv2D(
          filters=self._decoder_projected_depth,
          kernel_size=(1, 1),
          padding='same',
          kernel_regularizer=self._kernel_regularizer,
    ...
    if (low_res_height != self._output_size[0] or
      ...
      self._upsample_low_res = keras.layers.Resizing(
          self._output_size[0],
          self._output_size[1],
          interpolation=self._interpolation,
          crop_to_aspect_ratio=False)
    if (high_res_height != self._output_size[0] or
      ...
      self._upsample_high_res = keras.layers.Resizing(
          self._output_size[0],
          self._output_size[1],
          interpolation=self._interpolation,
          crop_to_aspect_ratio=False)
...
@keras.utils.register_keras_serializable(package='Vision')
class DecoderConcatMergeBlock(keras.layers.Layer):
  ...
  def build(
    ...
    initial_feature_conv = keras.layers.Conv2D(
    ...
    project_feature_conv = keras.layers.Conv2D(
        filters=self._decoder_projected_depth,
        kernel_size=(1, 1),
        padding='same',
        kernel_regularizer=self._kernel_regularizer,
