...
class VisionTransformer(keras.layers.Layer):
  ...
  def build(self, input_shape):
    self.patch_to_embed = keras.layers.Conv2D(
...
class PixelClassifier(keras.layers.Layer):
...
class PixelLinearClassifier(keras.layers.Layer):
  ...
  def __init__(self, encoder, num_classes, num_filters, **kwargs):
    ...
    self.linear_clas = keras.layers.Dense(
        num_classes,
        kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.01),
    )
    ...
    self.linear_trans = keras.layers.Dense(
        num_filters,
        kernel_initializer=keras.initializers.TruncatedNormal(stddev=0.01),
    )
