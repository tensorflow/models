    ...
    if isinstance(inputs, dict):
      ...
      attention_mask = tf.transpose(
        tf.concat(
            values=[
                tf.ones(
                    (1, tf.shape(attention_mask)[0]), tf.float32),
...
class PixelClassifier(tf.keras.Model):
...
class PixelLinearClassifier(tf.keras.Model):
  """Pixel classifier for finetuning, with additional
  layer norm and linear layer in the classification head.
  Uses the average of all token representations"""
  ...
  def call(self, inputs, training=False):
    ...
    attention_mask = tf.tile(tf.expand_dims(attention_mask, 2), [1, 1, self.num_filters])
    encoded = self.encoder(inputs)
    encoded = self.norm(self.activation(self.linear_trans(encoded)))
    encoded = self.dropout(encoded, training=training)
    
    mean_pooling = tf.reduce_sum(encoded[:, 1:, :] * attention_mask, axis=1) / mask_lengths
