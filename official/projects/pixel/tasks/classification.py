...
class PixelClassificationTask(base_task.Task):
  ...
  def build_model(self) -> tf.keras.Model:
    ...
    model = pixel.PixelLinearClassifier(encoder, self.task_config.num_classes, self.task_config.model.filters)
    h, w = self.task_config.input_size
    positions = h // self.task_config.patch_h * w // self.task_config.patch_w
    model({"label": tf.zeros((1,)), "pixel_values": tf.zeros((1, self.task_config.num_channels, h, w)), "attention_mask": tf.zeros((1, positions))})
    return model
