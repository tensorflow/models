...
class PositionalDecoder(keras.layers.Layer):
  ...
  Attributes:
    inputs: A `Dict[Text, keras.Input]` with `latent_output` and
  ...
  def __init__(self,
    ...
    Args:
      ...
      position_encoding_intializer_stddev:
        `stddev` of `keras.initializers.TruncatedNormal` used for the
        learned position embedding table kernel initializer.
      name:
        Sets the `keras.layers.Layer` name.
      **kwargs:
        Any keyword arguments to pass through to `keras.layers.Layer`.
    """
  ...
  def call(self, inputs, training=None):
    ...
    Args:
      inputs:
        A `Dict[Text, keras.Input]` with `latent_output` and
        `input_mask`. The shape of `latent_output` is shape
        `(z_index_dim, d_latents)` with dtype `tf.float32` and `input_mask` is
        shape `(None)` with dtype `tf.int32`.
      training:
