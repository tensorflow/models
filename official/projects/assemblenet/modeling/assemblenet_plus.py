...
def softmax_merge_peer_attentions(peers):
  ...
  initial_attn_weights = keras.initializers.TruncatedNormal(stddev=0.01)(
      [len(peers)])
  ...
  def call(self,
    ...
    for i, inp in enumerate(inputs):
      if inp.shape[h_channel_loc] != sm_size[0] or inp.shape[h_channel_loc + 1] != sm_size[1]:  # pylint: disable=line-too-long
        ...
        if use_5d_mode:
          inp = keras.layers.MaxPool3D([1, ratio, ratio], [1, ratio, ratio],
                                          padding='same')(
                                              inp)
        else:
          inp = keras.layers.MaxPool2D([ratio, ratio], ratio,
                                          padding='same')(
                                              inp)
...
class AssembleNetPlus(keras.Model):
  ...
  def __init__(self,
    ...
    if use_object_input:
      original_inputs = keras.Input(shape=input_specs[0].shape[1:])
      object_inputs = keras.Input(shape=input_specs[1].shape[1:])
      input_specs = input_specs[0]
    else:
      original_inputs = keras.Input(shape=input_specs.shape[1:])
      object_inputs = None
...
class AssembleNetPlusModel(keras.Model):
