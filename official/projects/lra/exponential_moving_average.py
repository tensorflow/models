...
class MultiHeadEMA(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    self.damping_factor = self.add_weight(shape=(self.kernel_dim, self.ndim, 1),
                                          initializer="random_normal",
                                          trainable=True,
                                          name="damping_factor",
                                          dtype=tf.float32)
    self.decay_factor = self.add_weight(shape=(self.kernel_dim, self.ndim, 1),
                                        initializer="random_normal",
                                        trainable=True,
                                        name="decay_factor",
                                        dtype=tf.float32)
    self.ema_expansion_matrix = self.add_weight(shape=(self.kernel_dim,
                                                       self.ndim, 1),
                                                initializer="random_normal",
                                                trainable=True,
    ...
    self.kernel_projection_matrix = self.add_weight(
        ...
        dtype=tf.float32)
    self.residual_weight = self.add_weight(shape=(self.embed_dim,),
                                           initializer="ones",
                                           trainable=True,
  ...
  def _compute_kernel(self, length: int):
    ...
    vander = tf.cast(
        ...
        dtype=damping_factor.dtype) * tf.math.log(previous_timestep_weight)
    kernel = (damping_factor * self.ema_expansion_matrix) * tf.math.exp(vander)
    # D x L
    return tf.einsum("dnl,dn->dl", kernel,
  ...
  def kernel(self, length: int):
    ...
    kernel_size = length if self.truncation is None else min(
        self.truncation, length)
  ...
  def call(self, x, padding_mask: Optional[tf.Tensor] = None) -> tf.Tensor:
    ...
    if self.bidirectional:
      ...
      # D x 2*L-1
      paddingL = tf.constant([[0, 0], [kernel_size - 1, 0]])
      paddingR = tf.constant([[0, 0], [0, kernel_size - 1]])
      paddingX = tf.constant([[0, 0], [0, 0], [kernel_size - 1, 0]])
      k = tf.pad(k1, paddingL) + tf.pad(tf.reverse(k2, axis=[-1]), paddingR)
      x = tf.pad(x, paddingX)
      fft_len = fft_len + kernel_size - 1
      s = 2 * kernel_size - 2

    k_f = tf.signal.rfft(k,
                         fft_length=tf.constant([2 * fft_len], dtype=tf.int32))
    x_f = tf.signal.rfft(x,
                         fft_length=tf.constant([2 * fft_len], dtype=tf.int32))
    # B x D x L
    out = tf.signal.irfft(x_f * k_f,
                          fft_length=tf.constant([2 * fft_len],
                                                 dtype=tf.int32))[...,
                                                                  s:s + seq_len]
