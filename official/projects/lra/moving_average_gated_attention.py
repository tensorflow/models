...
class RelativePositionBias(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    self.rel_pos_bias = tf.Variable(gauss_init(
        shape=[2 * self.max_positions - 1], dtype=tf.float32),
                                    trainable=True)
...
class MovingAverageGatedAttention(tf.keras.layers.Layer):
  ...
  def __init__(
    ...
    self.move = MultiHeadEMA(embed_dim,
                             ndim=ndim,
  ...
  def build(self, input_shape):
    ...
    self._intermediate_dense = tf.keras.layers.Dense(self.inner_dim,
                                                     use_bias=True)
    ...
    self.gamma = tf.Variable(gauss_init(shape=[2, self.zdim], dtype=tf.float32),
                             trainable=True)
    self.beta = tf.Variable(zero_init(shape=[2, self.zdim], dtype=tf.float32),
                            trainable=True)
  ...
  def call(self, inputs: Any) -> Any:
    ...
    h = tf.transpose(tf.reshape(tf.linalg.matmul(kernel, v),
                                shape=(bsz, seq_len, self.hdim)),
                     perm=(1, 0, 2))
