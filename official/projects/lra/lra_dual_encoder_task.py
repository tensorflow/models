...
class DualEncoderTask(base_task.Task):
  ...
  def build_losses(self, labels, model_outputs, aux_losses=None) -> tf.Tensor:
    ...
    if self.task_config.model.num_classes == 1:
      ...
      loss = tf.keras.losses.sparse_categorical_crossentropy(label_ids,
                                                             tf.cast(
                                                                 model_outputs,
                                                                 tf.float32),
                                                             from_logits=True)
  ...
  def build_inputs(self, params, input_context=None):
    ...
    if params.input_path == 'dummy':
      ...
      def dummy_data(_):
        ...
        x = dict(left_word_ids=dummy_ids,
                 left_mask=dummy_ids,
...
def predict(task: DualEncoderTask,
  ...
  if params_aug is not None:
    ...
    if is_regression:
      ...
      return [
          tf.argmax((1. - test_time_aug_wgt) * x[1] + test_time_aug_wgt * y[1],
