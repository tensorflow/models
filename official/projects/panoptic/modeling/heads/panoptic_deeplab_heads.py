...
class PanopticDeeplabHead(keras.layers.Layer):
  ...
  def build(self, input_shape: Union[tf.TensorShape, List[tf.TensorShape]]):
    ...
    random_initializer = keras.initializers.RandomNormal(stddev=0.01)
...
class SemanticHead(PanopticDeeplabHead):
  ...
  def build(self, input_shape: Union[tf.TensorShape, List[tf.TensorShape]]):
    ...
    self._classifier = keras.layers.Conv2D(
...
class InstanceHead(PanopticDeeplabHead):
  ...
  def build(self, input_shape: Union[tf.TensorShape, List[tf.TensorShape]]):
    ...
    self._instance_center_prediction_conv = keras.layers.Conv2D(
    ...
    self._instance_center_regression_conv = keras.layers.Conv2D(
