...
The design splits configuration into two kinds of value:
  ...
  * Operator knobs live in ``config.yaml`` and may be edited by hand by
    anyone, technical or not. Paths, the active prompt, split ratios, and the
    per-prompt detection thresholds and augmentation lists all live there.
...
# Every crop variant segment_to_classifier.py knows how to write.
...
class DetectionConfig:
  """SAM3 detection thresholds and sizing for one prompt.
  ...
  Attributes:
      ...
      containment_threshold: Ratio above which a smaller mask is treated as
        contained by a larger one and removed.
...
@dataclasses.dataclass(frozen=True)
class PromptConfig:
  """Everything that depends on the object class for one prompt.

  Attributes:
      detection: Detection thresholds and sizing for this prompt.
...
class PipelineConfig:
  ...
  @property
  def active_prompt(self) -> PromptConfig:
    """Returns the :class:`PromptConfig` for ``prompt_to_detect``."""
    return self.prompts[self.prompt_to_detect]

  @property
  def active_detection(self) -> DetectionConfig:
    """Returns the :class:`DetectionConfig` for ``prompt_to_detect``."""
    return self.prompts[self.prompt_to_detect].detection

  @property
  def active_augmentations(self) -> tuple[str, ...]:
    """Returns the augmentation tuple for ``prompt_to_detect``."""
    return self.prompts[self.prompt_to_detect].augmentations
...
def _validate_detection(
  ...
  max_short_side = _require_positive_int(
      raw_detection["max_short_side"], f"{context}.max_short_side"
  )
  crop_size = _validate_crop_size(raw_detection["crop_size"], prompt_name)
