...
"""Utility functions for preprocessing, RFDETR adapter, and postprocessing.
...
relevant thresholds and sizes in explicitly. Grouped roughly into:
  ...
  * Image resize
  * RFDETR adapter (converts a ``supervision.Detections`` into the tensor-
    dict shape the rest of the pipeline uses)
...
The tensor-dict shape used throughout is intentionally the same one the
prior SAM3-based pipeline used, so the post-processing filters and crop
helpers below did not need to change when the detector was swapped. Keys:

  * ``masks``           torch.bool ``[N, 1, H, W]``
  * ``masks_logits``    torch.float32 ``[N, 1, H, W]`` (zeros; unused)
  * ``boxes``           torch.float32 ``[N, 4]``  (x_min, y_min, x_max, y_max)
  * ``scores``          torch.float32 ``[N]``
  * ``original_height`` int
  * ``original_width``  int
"""
...
_STATE_META_KEYS = ("original_height", "original_width")
...
def convert_rfdetr_detections_to_state(
    detections: sv.Detections,
    image_height: int,
    image_width: int,
) -> dict[str, Any]:
  """Converts a ``supervision.Detections`` into the pipeline's state dict.

  The pipeline-internal state layout is intentionally identical to what the
  prior SAM3-based pipeline produced, so the downstream post-processing
  filters and crop helpers work unchanged.

  ``class_id`` from the RFDETR detections is intentionally discarded: the
  pipeline uses the dataset-folder name as the class label, not the
  detector's per-detection prediction.

  Args:
      detections: A ``supervision.Detections`` returned by ``RFDETR.predict``.
      image_height: Height of the image that produced the detections.
      image_width: Width of the image that produced the detections.

  Returns:
      A state dict with keys ``masks``, ``masks_logits``, ``boxes``,
      ``scores``, ``original_height``, ``original_width``. ``masks_logits``
      is filled with zeros of the same shape as ``masks`` since RFDETR does
      not expose per-pixel logits; downstream filters do not read this entry.
  """
...
def filter_contained_sub_masks(
  ...
  Args:
      state: Dict with keys ``'masks'``, ``'masks_logits'``, ``'boxes'``,
        ``'scores'``. ``masks`` is a bool tensor of shape ``[N, 1, H, W]``
        or ``[N, H, W]``.
...
def get_valid_bottle_indices(
  ...
  """Filters detections to remove edge objects less than 50% visible.
  ...
  The name ``bottle`` is legacy from an earlier project; the filter is
  class-agnostic.

  Args:
      detection_state: State dict with keys ``'boxes'``, ``'masks'``,
  ...
  Returns:
      A filtered state dict with partially visible edge detections removed.
  """
  boxes = detection_state["boxes"].numpy()
  masks = detection_state["masks"].numpy()
  if masks.ndim == 4:
    masks = masks.squeeze(1)

  image_height = detection_state["original_height"]
  image_width = detection_state["original_width"]
  ...
  filtered_output = {}
  for key in _STATE_META_KEYS:
    filtered_output[key] = detection_state[key]
  for key in _STATE_ARRAY_KEYS:
    filtered_output[key] = detection_state[key][valid_indices]
...
def merge_contained_boxes(
  ...
  Args:
      state: State dict with ``'masks'``, ``'boxes'``, ``'scores'`` keys.
        ``masks`` is expected to have shape ``[N, 1, H, W]``; the function
        squeezes the channel dim internally.
      containment_threshold: Minimum fraction of the smaller box's area that
        must overlap with the larger box to trigger a merge.

  Returns:
      A state dict with merged detections. The output ``masks`` tensor has
      shape ``[N_merged, 1, H, W]`` to match the input layout.
  """
  masks = state["masks"]
  boxes = state["boxes"]
  scores = state["scores"]
...
def crop_masked_image(
  ...
  Args:
      ...
      box: Bounding box as ``[x_min, y_min, x_max, y_max]``.
      size: Output size after letterboxing.
      background_color: RGB tuple used for the background outside the mask
        and for the letterbox padding. Defaults to black to preserve the
        previous behavior for any other caller.
...
def display_crop_thumbnails(
  ...
  Args:
      ...
      state: State dict containing ``'scores'``.
