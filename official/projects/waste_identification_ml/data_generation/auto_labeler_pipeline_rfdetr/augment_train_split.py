...
survives any future re-augmentation.

Which augmentations are applied is controlled by ``config.augmentations``.
Output filenames always follow the canonical augmentation order defined in
...
def process_target_folder(
  ...
  Raises:
      ValueError: If a mask's size does not match its corresponding image size.
      FileNotFoundError: If any image in the target folder is missing its
        matching mask sidecar.
  """
...
def main(config_path: str = _DEFAULT_CONFIG_PATH) -> None:
  ...
  augmentations_to_apply = config.augmentations
