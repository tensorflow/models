...
"""Batch RFDETR segmentation pipeline that writes a classifier-ready dataset.

Discovers dataset subfolders under a single root directory, runs RFDETR
...
RFDETR's per-detection class predictions are intentionally discarded: the
class label of every crop is the dataset folder name, matching the
one-class-per-folder convention of the prior SAM3-based pipeline. RFDETR is
used purely as the "find and segment objects" engine.

For every saved crop, a matching binary mask is written as a sibling PNG
with a ``_mask.png`` suffix (e.g. ``image_001_0.jpg`` +
...
restricted to the foreground object.
...
# Suffix for the mask sidecar written next to every crop. The augmentation
# stage looks for this exact suffix.
...
def get_background_color_for_variant(
  ...
  """Returns the background RGB color used by the given crop variant.
...
def build_variant_crop(
  ...
  """
  if variant == _RAW_VARIANT:
    return detection_utils.crop_raw_masked_image(image_array, mask, box)
  if variant == _BLACK_BACKGROUND_VARIANT:
    return detection_utils.crop_masked_image(
        image_array, mask, box, size=crop_size
    )
  if variant == _IMAGENET_MEAN_BACKGROUND_VARIANT:
    return detection_utils.crop_with_mean_background_blend(
        image_array,
        mask,
        box,
        size=crop_size,
...
def build_variant_mask(
  ...
  """
  if variant == _RAW_VARIANT:
    return detection_utils.build_raw_variant_mask(mask, box)
  if variant in (_BLACK_BACKGROUND_VARIANT, _IMAGENET_MEAN_BACKGROUND_VARIANT):
    return detection_utils.build_letterboxed_variant_mask(
...
def generate_selected_crops(
  ...
  for detection_index in range(num_detections):
    ...
    mask = detection_utils.fill_mask_holes(mask)
...
def save_one_detection(
  ...
  Args:
      ...
      write_masks: If ``True``, write ``<name>_mask.png`` sidecars alongside
        each crop. If ``False``, only the crop JPEGs are written and the masks
        in ``variant_to_mask`` are discarded.
  """
...
def _postprocess_detections(
  ...
  """Applies the standard post-inference filters to a detection state.

  Args:
      state: Raw detection state dict from the RFDETR adapter.
...
def process_split(
  ...
  Args:
      ...
      write_masks: Whether to compute and write ``_mask.png`` sidecars for this
        ...
        them).
  """
  variant_directories = build_variant_directories(
      class_folder, config.crop_variants
  )
  ...
  with futures.ThreadPoolExecutor(
    ...
    for image_path in tqdm.tqdm(image_paths, desc=log_label):
      ...
      state = _postprocess_detections(state, config)
      ...
      if len(pending_futures) >= config.queue_maxsize + 1:
...
def process_dataset(
  ...
  for split_name in split_names:
    ...
    class_folder = os.path.join(config.classifier_dir, split_name, dataset_name)
