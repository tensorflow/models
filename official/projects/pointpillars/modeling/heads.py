...
class SSDHead(keras.layers.Layer):
  ...
  def build(self, input_specs: Mapping[str, tf.TensorShape]):
    ...
    self._box_regressor = keras.layers.Conv2D(
    ...
    if self._config_dict['attribute_heads']:
      ...
      for att_config in self._config_dict['attribute_heads']:
        ...
        self._att_predictors[att_name] = keras.layers.Conv2D(
  ...
  @classmethod
  def from_config(cls, config: Mapping[str, Any]) -> keras.layers.Layer:
    return cls(**config)
