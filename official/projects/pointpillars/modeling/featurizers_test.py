...
class FeaturizerTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_network_creation(self, image_size, pillars_size, train_batch_size,
    ...
    # Train mode.
    pillars = keras.Input(shape=pillars_size, batch_size=train_batch_size)
    indices = keras.Input(
    ...
    # Evaluation mode.
    pillars = keras.Input(shape=pillars_size, batch_size=eval_batch_size)
    indices = keras.Input(
    ...
    pillars = keras.Input(shape=pillars_size, batch_size=1)
    indices = keras.Input(
