...
from official.projects.pointpillars.utils import wod_processor
...
class FrameDecoder(decoder.Decoder):
  """The class to decode a raw serialized frame to tensors.

  Note: The decoding from raw data way is just a experimental trial. It runs
  too slowly to afford an online data pipeline, as it uses tf.py_function to
  call the data processor. Do not use this decoder in production environment.
  ...
  def __init__(self,
               ...
               pillars_config: cfg.PillarsConfig):
    self._pillars_config = pillars_config
    self._wod_processor = wod_processor.WodProcessor(image_config,
                                                     pillars_config)

  def _decode_frame(self, raw_frame) -> Any:
    frame = dataset_pb2.Frame()
    frame.ParseFromString(bytearray(raw_frame.numpy()))
    return self._wod_processor.process_one_frame(frame)

  def decode(self, raw_frame: Any) -> Mapping[str, Any]:
    (timestamp, pillars, indices, classes, ymin, xmin, ymax, xmax, heading, z,
     height,
     difficulty) = tf.py_function(self._decode_frame, [raw_frame], [
         tf.int64, tf.float32, tf.int32, tf.int32, tf.float32, tf.float32,
         tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.int32
     ], 'decode_frame')

    tf.ensure_shape(timestamp, [])
    tf.ensure_shape(pillars, [
        self._pillars_config.num_pillars,
        self._pillars_config.num_points_per_pillar,
        self._pillars_config.num_features_per_point
    ])
    tf.ensure_shape(indices, [self._pillars_config.num_pillars, 2])
    tf.ensure_shape(classes, [None])
    tf.ensure_shape(ymin, [None])
    tf.ensure_shape(xmin, [None])
    tf.ensure_shape(ymax, [None])
    tf.ensure_shape(xmax, [None])
    tf.ensure_shape(heading, [None])
    tf.ensure_shape(z, [None])
    tf.ensure_shape(height, [None])
    tf.ensure_shape(difficulty, [None])

    boxes = tf.stack([ymin, xmin, ymax, xmax], axis=-1)
    attr_heading = tf.expand_dims(heading, axis=1)
    attr_z = tf.expand_dims(z, axis=1)
    attr_height = tf.expand_dims(height, axis=1)
    decoded_tensors = {
        'frame_id': timestamp,
        'pillars': pillars,
        'indices': indices,
