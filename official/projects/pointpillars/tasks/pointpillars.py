...
class PointPillarsTask(base_task.Task):
  ...
  def build_inputs(
    ...
    if self.task_config.use_raw_data:
      decoder = decoders.FrameDecoder(model_config.image,
                                      model_config.pillars)
    else:
      decoder = decoders.ExampleDecoder(model_config.image,
                                        model_config.pillars)
