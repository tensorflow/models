...
class LongformerAttention(keras.layers.MultiHeadAttention):
  """LongformerAttention.
  ...
  def _build_from_signature(self, query, value, key=None):
    ...
    self._query_dense = keras.layers.EinsumDense(
        ...
        name="query",
        **common_kwargs)
    self._global_query_dense = keras.layers.EinsumDense(
        einsum_equation,
        output_shape=_get_output_shape(output_rank - 1,
                                       [self._num_heads, self._key_dim]),
        bias_axes=bias_axes if self._use_bias else None,
    ...
    self._key_dense = keras.layers.EinsumDense(
        ...
        name="key",
        **common_kwargs)
    self._global_key_dense = keras.layers.EinsumDense(
        einsum_equation,
        output_shape=_get_output_shape(output_rank - 1,
                                       [self._num_heads, self._key_dim]),
        bias_axes=bias_axes if self._use_bias else None,
    ...
    self._value_dense = keras.layers.EinsumDense(
        ...
        name="value",
        **common_kwargs)
    self._global_value_dense = keras.layers.EinsumDense(
        einsum_equation,
        output_shape=_get_output_shape(output_rank - 1,
                                       [self._num_heads, self._value_dim]),
        bias_axes=bias_axes if self._use_bias else None,
    ...
    self._global_dropout_layer = keras.layers.Dropout(rate=self._dropout)
