...
"""Transformer-based BERT encoder network for FFFNER."""
...
class FFFNerEncoder(tf.keras.Model):
  ...
  *Note* that the network is constructed by
  ...
  def __init__(
    ...
    cls_token_pos = tf.keras.layers.Input(
        shape=(None,), dtype=tf.int32, name='cls_token_pos')
    span_start_pos = tf.keras.layers.Input(
        shape=(None,), dtype=tf.int32, name='span_start_pos')
    ...
    encoder_output_1 = tf.gather(last_encoder_output, indices=cls_token_pos, axis=1, batch_dims=1)
    encoder_output_2 = tf.gather(last_encoder_output, indices=span_start_pos, axis=1, batch_dims=1)
    pooler_layer_1 = tf.keras.layers.Dense(
        units=hidden_size,
        activation='tanh',
        kernel_initializer=initializer,
        name='pooler_transform_1')
    pooler_layer_2 = tf.keras.layers.Dense(
        units=hidden_size,
        activation='tanh',
        kernel_initializer=initializer,
        name='pooler_transform_2')
    cls_output_1 = pooler_layer_1(encoder_output_1)
    cls_output_2 = pooler_layer_2(encoder_output_2)
    cls_output = tf.concat([cls_output_1, cls_output_2], 1)
    outputs = dict(
    ...
    if dict_outputs:
      super().__init__(
          inputs=[word_ids, mask, type_ids, cls_token_pos, span_start_pos], outputs=outputs, **kwargs)
    else:
      ...
      super().__init__(  # pylint: disable=bad-super-call
          inputs=[word_ids, mask, type_ids, cls_token_pos, span_start_pos],
          outputs=outputs,
          **kwargs)

    self._pooler_layer_1 = pooler_layer_1
    self._pooler_layer_2 = pooler_layer_2
  ...
  def pooler_layer(self):
    ...
    assert False, "why access me??"
    return self._pooler_layer_1
