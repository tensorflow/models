...
class NERDataset:
  ...
  def process_to_input(self,
                       input_ids,
                       is_entity_token_pos,
                       entity_type_token_pos,
                       is_entity_label,
                       entity_type_label,
                       sid,
                       span_start,
  ...
  def process_word_list_and_spans_to_inputs(self, sid, word_list, spans):
    ...
    for span_start, span_end, span_type, span_label in spans:
      ...
      yield self.process_to_input(input_ids,
                                  is_entity_token_pos,
                                  entity_type_token_pos,
                                  is_entity_label,
                                  entity_type_label,
  ...
  def bio_labels_to_spans(self, bio_labels):
    ...
    spans = list(filter(
      lambda x: x[2] in self.label_to_entity_type_index.keys(), spans))
    return spans
  ...
  def prepare(self, negative_multiplier=3.):
    ...
    """
    desc = "prepare data for training" if \
      self.is_train else "prepare data for testing"
    ...
    for sid, (tokens, labels) in tqdm(enumerate(self.read_file()), desc=desc):
      ...
      entity_spans_dict = {(start, end): ent_type for
                           start, end, ent_type in entity_spans}
      num_entities = len(entity_spans_dict)
      num_negatives = int((len(tokens) + num_entities * 10) *
                          negative_multiplier)
          ...
          for i in range(len(tokens) - n_words + 1):
            ...
            if not self.is_train or ent_type != 'O':
              ...
              intersection_size = (is_token_entity_prefix[j + 1] -
                                   is_token_entity_prefix[i] + 1) / (j + 1 - i)
              negative_spans_probs.append(math.e ** intersection_size)

        if negative_spans and num_negatives > 0:
          negative_spans_probs = np.array(
            negative_spans_probs) / np.sum(negative_spans_probs)
          negative_span_indices = np.random.choice(len(negative_spans),
                                                   num_negatives,
                                                   replace=True,
...
if __name__ == '__main__':
  ...
  _label_to_entity_type_index = {k: i for i, k in
                                 enumerate(list(entity_map.keys()))}
  train_ds = NERDataset(words_path=os.path.join(dataset, train_file + ".words"),
  ...
  eval_ds = NERDataset(words_path=os.path.join(dataset, test_file + ".words"),
  ...
  def file_based_convert_examples_to_features(examples, output_file):
    ...
    for ex_index in range(len(examples["input_ids"])):
      ...
      features["example_id"] = create_int_feature(
        [examples["id"][ex_index]])
  ...
  file_based_convert_examples_to_features(
    train_data,
    f"{dataset_name}_{train_file}.tf_record")
  file_based_convert_examples_to_features(
    eval_data,
    f"{dataset_name}_{test_file}.tf_record")