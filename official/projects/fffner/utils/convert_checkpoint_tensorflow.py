...
def _get_tensorflow_bert_model_and_config(tfhub_handle_encoder):
  ...
  named_parameters = {n: p for n, p in zip(bert_model_weights_name, bert_model_weights)}
  # construct config from parameters, the get_config() function in bert_model does not return any such information
  config = {}
...
def convert_checkpoint(output_path, tfhub_handle_encoder):
  ...
  bert_model, bert_config = _get_tensorflow_bert_model_and_config(tfhub_handle_encoder)
...
def main(_):
  convert_checkpoint(output_path="tf-bert-uncased",
