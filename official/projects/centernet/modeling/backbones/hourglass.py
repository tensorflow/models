  ...
  def __init__(
    ...
    Args:
      ...
      input_channel_dims: `int`, number of filters used to downsample the
        input image.
      input_specs: A `keras.layers.InputSpec` of specs of the input tensor.
    ...
    inputs = keras.layers.Input(shape=input_specs.shape[1:])
    ...
    for i in range(num_hourglasses):
      ...
      if i < num_hourglasses - 1:
        ...
        x_downsampled = keras.layers.Add()([inter_hg_conv1, inter_hg_conv2])
        x_downsampled = keras.layers.ReLU()(x_downsampled)
