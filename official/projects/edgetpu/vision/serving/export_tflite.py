...
flags.DEFINE_bool(
    ...
    'tf.saved_model API; if True, export Keras SavedModel with keras.Model'
    'API.')
...
def run_export():
  ...
  if export_config.output_layer is not None:
    ...
    model = keras.Model(model.input, output_layer.output)
  ...
  model_for_inference = keras.Model(model_input, model_output)
