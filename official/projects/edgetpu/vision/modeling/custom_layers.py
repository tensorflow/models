...
class GroupConv2D(keras.layers.Conv2D):
  ...
  def __init__(self,
               ...
               batch_norm_layer: Optional[keras.layers.Layer] = None,
               bn_epsilon: float = 1e-3,
               bn_momentum: float = 0.99,
               **kwargs: Any) -> keras.layers.Layer:
    ...
    Args:
      ...
      batch_norm_layer: The batch normalization layer to use. This is typically
        keras.layer.BatchNormalization or a derived class.
...
class GroupConv2DKerasModel(keras.Model):
  ...
  def __init__(self,
    ...
    if 'activation' in kwargs.keys():
      self.activation = keras.activations.get(kwargs['activation'])
    ...
    for _ in range(self._groups):
      ...
      self.conv_layers.append(
          keras.layers.Conv2D(per_conv_filter_size, kernel_size, **kwargs))
...
class ArgmaxKerasLayer(keras.layers.Layer):
