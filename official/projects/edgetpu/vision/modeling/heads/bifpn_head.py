...
def build_batch_norm(is_training_bn: bool,
  ...
  if is_training_bn:
    ...
    batch_norm_class = keras.layers.BatchNormalization
...
def get_conv_op(conv_type):
  ...
  if conv_type.startswith('sep'):
    ...
    conv_op = functools.partial(
        keras.layers.Conv2D, kernel_size=(kernel_size, kernel_size))
  else:
...
class FNode(keras.layers.Layer):
...
class FPNCells(keras.layers.Layer):
  """FPN cells."""
...
class FPNCell(keras.layers.Layer):
  """A single FPN cell."""
...
class SegClassNet(keras.layers.Layer):
  ...
  def __init__(self,
    ...
    if fullres_output:
      ...
      for i in reversed(range(min_level)):
        ...
        self.fullres_conv_transpose[str(i)] = keras.layers.Conv2DTranspose(
            filters=num_filters,
            data_format=data_format,
            kernel_size=3,
            strides=2,
    ...
    if self.fullres_output:
      for i in reversed(range(self.min_level)):
        if self.fullres_skip_connections:
          net = keras.layers.Concatenate()([net, backbone_feats[i + 1]])
