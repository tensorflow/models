...
# This function is directly copied from the keras.layers.MultiHeadAttention
# implementation.
...
class EdgeTPUSoftmax(keras.layers.Softmax):
  ...
  def call(self, inputs, mask=None):
    ...
    if isinstance(self.axis, (tuple, list)):
      if len(self.axis) > 1:
        ...
        return keras.backend.softmax(inputs, axis=self.axis[0])
    return keras.backend.softmax(inputs, axis=self.axis)


@keras.utils.register_keras_serializable(package='Text')
class EdgeTPUMultiHeadAttention(keras.layers.MultiHeadAttention):
