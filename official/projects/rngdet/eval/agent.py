...
class Agent(FrozenClass):
    def __init__(self, model, image):
        ...
        self.crop_size = 128
        self.pad_size = 128
        self.extract_candidate_threshold = 0.7
        self.candidate_filter_threshold = 30
        self.alignment_distance = 5
    ...
    def get_candidate_initial_vertices_from_local_peaks(self):
        ...
        Image.fromarray(binary_mask.astype(np.uint8)).convert('RGB').save(f'./segmentation/0_segment.png')
        Image.fromarray(point_mask.astype(np.uint8)).convert('RGB').save(f'./segmentation/0_point.png')
