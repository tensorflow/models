...
class Edge(FrozenClass):
    def __init__(self,args,id,src,dst,vertices,orientation):
        self.args = args
        self.id = id
        # init and end
        self.src = src
    ...
    def step_BC(self,init_step=False):
        edge_move_ahead_length = self.args.edge_move_ahead_length if not init_step else self.args.edge_move_ahead_length // 2
        # find index now
...
class Sampler(FrozenClass):
    def __init__(self,args,image_name):
        # args
        self.args = args
    ...
    def load_data_and_initialization(self,tile_name):
        ...
        # segment_mask
        self.label_masks[:,:,0] = np.array(Image.open(os.path.join(self.args.dataroot,f'segment/{tile_name}.png')))[:,:,0]
        # point_mask
        self.label_masks[:,:,1] = np.array(Image.open(os.path.join(self.args.dataroot,f'intersection/{tile_name}.png')))[:,:,0]
        self.label_masks = np.pad(self.label_masks,np.array(((self.pad_size,self.pad_size),(self.pad_size,self.pad_size),(0,0))),'constant')
        
        # load graph
        with open(os.path.join(self.args.dataroot,f'graph/{tile_name}.json'),'r') as jf:
        ...
        for i, e in enumerate(edges):
            ...
            new_edge = Edge(self.args,e['id'],src,dst,e['vertices'],e['orientation'])
    ...
    def step_expert_BC_sampler(self):
        ...
        # edge mode, find vertex in the next step
            ...
            if self.current_e.finished:
                ...
                # add unique noise
                v_next_noise = np.array(v_next) + np.random.uniform(-1, 1, 2)*self.args.noise
                v_next_noise = [int(max(0,min(self.args.image_size-1,x))) for x in v_next_noise]
