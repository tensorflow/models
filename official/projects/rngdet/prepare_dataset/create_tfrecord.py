...
flags.DEFINE_integer('roi_size', 128, 'Path to output file')
flags.DEFINE_integer('image_size', 2048, 'Path to output file')
flags.DEFINE_integer('edge_move_ahead_length', 30, 'Path to output file')
flags.DEFINE_integer('num_queries', 10, 'Path to output file')
flags.DEFINE_integer('noise', 8, 'Path to output file')
flags.DEFINE_integer('max_num_frame', 10000, 'Path to output file')
...
def generate_samples(dataroot, idx, image_size, roi_size, num_queries,
                     noise, edge_move_ahead_length, max_num_frame):
  # iterate training data
  with open(f'./{dataroot}/data_split_{idx}.json','r') as jf:
...
def main(_):
  ...
  cityscale_annotations_iter = generate_samples(FLAGS.dataroot,
                                                ...
                                                FLAGS.max_num_frame)
  prefix = f'./{FLAGS.output_file_prefix}-noise-{FLAGS.noise}-{idx}'
