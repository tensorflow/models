...
class Edge(FrozenClass):
    def __init__(self,edge_move_ahead_length,id,src,dst,vertices,orientation):
        ...
        self.tracked = False
        self.finished = False
        self.v_now = tf.identity(vertices[0])
        self.max_index = 0
        # data
        self.vertices = vertices
        self.raw_vertices = tf.identity(vertices)
        self.orientation = orientation
        self.raw_orientation = tf.identity(orientation)
        self.ahead_segment = []
    ...
    def reverse(self):
        ...
        self.v_now = tf.identity(self.vertices[0])
        self.max_index = 0
        self.ahead_segment = []
...
class Sampler(FrozenClass):
  ...
  def load_data_and_initialization(self):
    ...
    # segment_mask
    self.label_masks[:,:,0] = self.segment_image[:,:,0]
    # point_mask
    self.label_masks[:,:,1] = self.intsec_image[:,:,0]
    self.label_masks = np.pad(self.label_masks,np.array(((self.pad_size,self.pad_size),(self.pad_size,self.pad_size),(0,0))),'constant')
    # load graph
    v_id = self.data_v['id']
    v_x = self.data_v['x']
    v_y = self.data_v['y']
    for id, x, y in zip(v_id, v_x, v_y):
      new_v = Vertex(id,x,y)
      self.vertices[id] = new_v
      
    e_id = self.data_e['id']
    e_src = self.data_e['src']
    e_dst = self.data_e['dst']
    e_vertices = self.data_e['vertices']
    e_vertices = tf.reshape(e_vertices, [-1, 2])
    e_orientation = self.data_e['orientation']
    e_length = self.data_e['length']
    offset = 0
    for i in range(len(e_id)):
      src, dst = self.vertices[e_src[i]], self.vertices[e_dst[i]]
      #print(e_vertices)
      #print("-------------------------------------------")
      #print(e_orientation)
      vertices = e_vertices[offset:offset+e_length[i],:]
      orientation = e_orientation[offset:offset+e_length[i]]
      new_edge = Edge(
          self.edge_move_ahead_length, e_id[i], src, dst, vertices, orientation)
      self.edges[e_id[i]] = new_edge
  ...
  def step_expert_BC_sampler(self):
      ...
      # edge mode, find vertex in the next step
          ...
          v_next = self.current_e.step_BC().numpy()
          v_nexts = [v_next]
