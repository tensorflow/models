...
class BertDistillationTask(policies.ProgressivePolicy, base_task.Task):
  ...
  # override policies.ProgressivePolicy
  def get_model(self, stage_id, old_model=None) -> keras.Model:
    del old_model
  ...
  def get_optimizer(self, stage_id):
    ...
    if isinstance(optimizer, keras.optimizers.experimental.Optimizer):
      optimizer = keras.__internal__.optimizers.convert_to_legacy_optimizer(
          optimizer)
  ...
  def build_model(self, stage_id) -> keras.Model:
  ...
  def _get_attention_loss(self, teacher_score, student_score):
    ...
    # the original one (keras.losses.KLDivergence). We adopt this approach
    # to stay consistent with the TF1 implementation.
    teacher_weight = keras.activations.softmax(teacher_score, axis=-1)
  ...
  def build_losses(self, labels, outputs, metrics) -> tf.Tensor:
    ...
    if not last_stage:
      ...
      feature_transfer_loss = keras.losses.mean_squared_error(
  ...
  def build_metrics(self, training=None):
    del training
    metrics = [
        keras.metrics.Mean(name='feature_transfer_mse'),
        keras.metrics.Mean(name='beta_transfer_loss'),
        keras.metrics.Mean(name='gamma_transfer_loss'),
        keras.metrics.SparseCategoricalAccuracy(name='masked_lm_accuracy'),
        keras.metrics.Mean(name='lm_example_loss'),
        keras.metrics.Mean(name='total_loss')]
    if self._progressive_config.layer_wise_distill_config.if_transfer_attention:
      metrics.append(keras.metrics.Mean(name='attention_transfer_loss'))
    if self._task_config.train_data.use_next_sentence_label:
      metrics.append(keras.metrics.SparseCategoricalAccuracy(
          name='next_sentence_accuracy'))
      metrics.append(keras.metrics.Mean(name='next_sentence_loss'))
  ...
  # overrides base_task.Task
  def train_step(self, inputs, model: keras.Model,
                 optimizer: keras.optimizers.Optimizer, metrics):
  ...
  # overrides base_task.Task
  def validation_step(self, inputs, model: keras.Model, metrics):
    """Validatation step.
