...
class TestRunner(standard_runner.StandardTrainer,
  ...
  def __init__(self, return_numpy=False):
    ...
    self.optimizer = keras.optimizers.RMSprop(learning_rate=0.1)
    self.global_step = self.optimizer.iterations
    self.train_loss = keras.metrics.Mean("train_loss", dtype=tf.float32)
    self.eval_loss = keras.metrics.Mean("eval_loss", dtype=tf.float32)
  ...
  def train_step(self, iterator):
    ...
    def _replicated_step(inputs):
      ...
      with tf.GradientTape() as tape:
        ...
        loss = tf.reduce_mean(keras.losses.MSE(targets, outputs))
...
class TestEvaluatorWithNestedSummary(standard_runner.StandardEvaluator):
  ...
  def __init__(self):
    ...
    self.loss = keras.metrics.Mean("loss", dtype=tf.float32)
    self.accuracy = keras.metrics.CategoricalAccuracy(
        "accuracy", dtype=tf.float32)
    self.loss2 = keras.metrics.Mean("loss", dtype=tf.float32)
    self.accuracy2 = keras.metrics.CategoricalAccuracy(
        "accuracy", dtype=tf.float32)
  ...
  def eval_step(self, iterator):
    ...
    def _replicated_step(loss, accuracy, inputs):
      ...
      inputs, targets = inputs
      outputs = self.model(inputs)
      loss.update_state(keras.losses.MSE(targets, outputs))
