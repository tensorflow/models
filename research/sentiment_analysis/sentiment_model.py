...
class CNN(tf.keras.models.Model):
  ...
  def __init__(self, emb_dim, num_words, sentence_length, hid_dim, class_dim, dropout_rate):
    ...
    Args:
      ...
      num_words: The number of the most frequent tokens to be used from the corpus.
    ...
    layer_conv3 = tf.keras.layers.Lambda(
        _dynamic_pooling, output_shape=_dynamic_pooling_output_shape)(layer_conv3)
    layer_conv3 = tf.keras.layers.Flatten()(layer_conv3)

    layer_conv2 = tf.keras.layers.Conv1D(hid_dim, 2, activation='relu')(layer)
    layer_conv2 = tf.keras.layers.Lambda(
        _dynamic_pooling, output_shape=_dynamic_pooling_output_shape)(layer_conv2)
