...
[![Paper](http://img.shields.io/badge/Paper-arXiv.1704.04861-B3181B?logo=arXiv)](https://arxiv.org/abs/1704.04861)
[![Paper](http://img.shields.io/badge/Paper-arXiv.1801.04381-B3181B?logo=arXiv)](https://arxiv.org/abs/1801.04381)
[![Paper](http://img.shields.io/badge/Paper-arXiv.1905.02244-B3181B?logo=arXiv)](https://arxiv.org/abs/1905.02244)

This repository is the official implementation of the following papers.

* Paper title: [MobileNets: Efficient Convolutional Neural Networks for Mobile Vision Applications](https://arxiv.org/abs/1704.04861)
* Paper title: [MobileNetV2: Inverted Residuals and Linear Bottlenecks](https://arxiv.org/abs/1801.04381)
* Paper title: [Searching for MobileNetV3](https://arxiv.org/abs/1905.02244)

## Description

Mobilenets are small, low-latency, low-power models parameterized to meet the 
resource constraints of a variety of use cases. They can be built upon for classification, detection, embeddings 
and segmentation similar to how other popular large scale models, such as Inception, are used. 

In this folder we have provided the full model building codes for [MobileNetV1], [MobileNetV2] and [MobilenetV3] 
networks using [TF2.X with Keras] high level API. In particular, this module consists of
...
## Pretrained models
In the following, we have provided the pretrained checkpoints in TF1. To use them in TF2, please refer to 
[How to load TF1 trained checkpoints](#How-to-load-TF1-trained-checkpoints). Note that, currently MobileNet EdgeTPU
has not been implemented in TF2 yet.

Choose the right MobileNet model to fit your latency and size budget. The size of the network in memory and on disk 
is proportional to the number of parameters. The latency and power usage of the network scales with the number of 
MMadds. These MobileNet models have been trained on the ILSVRC-2012-CLS image classification dataset. 
Accuracies were computed by evaluating using a single image crop.

### Mobilenet V3 Imagenet TF1 Checkpoints
...
### Mobilenet V2 Imagenet TF1 Checkpoints
...
[configs/archs.py](configs/archs.py). One example of changing width_multiplier to 0.75 is as follows
```python
