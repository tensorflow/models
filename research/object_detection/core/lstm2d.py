...
import object_detection.core.lstm1d as lstm1d
...
def images_to_sequence2(tensor):
  """Convert a batch of images into a batch of sequences.
  ...
  Returns:
    (width, num_images*height, depth) sequence tensor
  """
  data_shape = tf.shape(tensor)
  num_image_batches = data_shape[0]
  height = data_shape[1]
  width = data_shape[2]
  depth = data_shape[3]
  #height, width, depth = tf.shape(tensor)
  transposed = array_ops.transpose(tensor, [2, 0, 1, 3])
  return array_ops.reshape(transposed, [width, num_image_batches * height, depth])
...
def sequence_to_images2(tensor, num_image_batches):
  """Convert a batch of sequences into a batch of images.
...
def horizontal_lstm2(images, num_filters_out, scope=None):
  """Run an LSTM bidirectionally over all the rows of each image.
  ...
  with variable_scope.variable_scope(scope, "HorizontalLstm", [images]):
    ...
    _, _, _, input_channels = _shape(images)
    sequence = images_to_sequence2(images)
    with variable_scope.variable_scope("lr"):
      hidden_sequence_lr = lstm1d.ndlstm_base2(sequence, num_filters_out // 2, shape[1], input_channels)
    with variable_scope.variable_scope("rl"):
      hidden_sequence_rl = (lstm1d.ndlstm_base2(
          sequence, num_filters_out - num_filters_out // 2, shape[1], input_channels, reverse=1))
    output_sequence = array_ops.concat([hidden_sequence_lr, hidden_sequence_rl],
                                       2)
    output = sequence_to_images2(output_sequence, batch_size)
    return output       
...
def separable_lstm(images, num_filters_out,
  ...
  """
  shape_ts = tf.shape(images)
  batch_size = images.get_shape()[0].value
  if batch_size is None:
      batch_size = shape_ts[0]
  ...
  with variable_scope.variable_scope(scope, "SeparableLstm", [images]):
    ...
    hidden = horizontal_lstm2(images, nhidden)      
  
    with variable_scope.variable_scope("vertical"):      
      transposed = array_ops.transpose(hidden, [0, 2, 1, 3])
      transposed = tf.reshape(transposed,[batch_size,shape_ts[2],shape_ts[1],num_filters_out])
      output_transposed = horizontal_lstm2(transposed, num_filters_out)
    output = array_ops.transpose(output_transposed, [0, 2, 1, 3])
    output = tf.reshape(output,[batch_size,shape_ts[1],shape_ts[2],num_filters_out])
    return output
...
if __name__ == "__main__":
    import numpy as np
    ph = tf.placeholder(tf.float32, shape=(None,None,None,3),name="input")
    input_val = tf.zeros(shape=(1, 10,8,3))
    outputs = separable_lstm(ph, 12, kernel_size=None, scope='lstm2d-1')    
    
    session = tf.Session()    
    init = [tf.global_variables_initializer(),tf.local_variables_initializer()]
    session.run(init)
    val = session.run(tf.shape(outputs),feed_dict={ph:np.zeros((1,10,8,3))})
    print(val)