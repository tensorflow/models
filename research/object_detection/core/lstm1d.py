...
def ndlstm_base_dynamic2(inputs, noutput, batch_size, input_channels, scope=None, reverse=False):
  """Run an LSTM, either forward or backward.
  ...
  with variable_scope.variable_scope(scope, "SeqLstm", [inputs]):
    ...
    lstm_cell = rnn_cell.BasicLSTMCell(noutput, state_is_tuple=False, activation = tf.nn.relu)
    state = array_ops.zeros([batch_size, lstm_cell.state_size])
    #sequence_length = int(inputs.get_shape()[0])
    ...
    inputs.set_shape([None, None, input_channels])
    #inputs = tf.reshape(inputs,[input_length,-1,input_channels])
...
def ndlstm_base2(inputs, noutput, batch_size, input_channels, scope=None, reverse=False, dynamic=True):
  """Implements a 1D LSTM, either forward or backward.
  ...
  # slim.rnn.basic_lstm_cell
  if dynamic:
    return ndlstm_base_dynamic2(inputs, noutput, batch_size, input_channels, scope=scope, reverse=reverse)
  else:
    return ndlstm_base_unrolled(inputs, noutput, scope=scope, reverse=reverse)
