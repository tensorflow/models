...
def rewrite_nn_resize_op(is_quantized=False):
  ...
  """
    ...
    reshape_2_pattern = graph_matcher.OpTypePattern(
        "Reshape",
        inputs=[graph_matcher.OneofPattern([fake_quant_pattern, mul_pattern]), "Const"],
        ordered_inputs=False,
    )
    add_type_name = "Add"
...
def add_output_tensor_nodes(
  ...
  """
    ...
    num_detections = tf.cast(
        postprocessed_tensors.get(detection_fields.num_detections), tf.int32
    )
    class_names = postprocessed_tensors.get(detection_fields.detection_class_names)
    outputs = {}
    outputs[detection_fields.detection_boxes] = tf.identity(
        boxes[:, :num_detections, ...], name=detection_fields.detection_boxes
    )
    outputs[detection_fields.detection_scores] = tf.identity(
        scores[:, :num_detections, ...], name=detection_fields.detection_scores
    )
    if multiclass_scores is not None:
        outputs[detection_fields.detection_multiclass_scores] = tf.identity(
            multiclass_scores[:, :num_detections, ...],
    ...
    if box_classifier_features is not None:
        outputs[detection_fields.detection_features] = tf.identity(
            box_classifier_features[:, :num_detections, ...],
            name=detection_fields.detection_features,
        )
    outputs[detection_fields.detection_classes] = tf.identity(
        classes[:, :num_detections, ...], name=detection_fields.detection_classes
    )
    ...
    if keypoints is not None:
        outputs[detection_fields.detection_keypoints] = tf.identity(
            keypoints[:, :num_detections, ...],
            name=detection_fields.detection_keypoints,
        )
    if masks is not None:
        outputs[detection_fields.detection_masks] = tf.identity(
            masks[:, :num_detections, ...], name=detection_fields.detection_masks
        )
    if class_names is not None:
        outputs[detection_fields.detection_class_names] = tf.identity(
            class_names[:, :num_detections, ...],
            name=detection_fields.detection_class_names,
        )
...
def _get_outputs_from_inputs(
    input_tensors, detection_model, output_collection_name, labels
):
    ...
    if labels is not None:
        idxs = tf.cast(
            postprocessed_tensors[fields.DetectionResultFields.detection_classes],
            tf.int32,
        )
        postprocessed_tensors[
            fields.DetectionResultFields.detection_class_names
        ] = tf.gather(labels, idxs)
...
def build_detection_graph(
    ...
    if input_shape is not None:
        if input_type != "image_tensor":
            raise ValueError(
                "Can only specify input shape for `image_tensor` " "inputs."
            )
...
def _get_labels(path_to_labels):
    """
    """
    label_map = label_map_util.load_labelmap(path_to_labels)
    categories = label_map_util.convert_label_map_to_categories(
        label_map, max_num_classes=float("Inf"), use_display_name=True
    )
    category_index = label_map_util.create_category_index(categories)
    sorted_category_names = [v["name"] for k, v in sorted(category_index.items())]
...
def export_inference_graph(
  ...
  """
    ...
    labels = _get_labels(path_to_labels) if path_to_labels is not None else None
