...
def return_checkpoints(dir_path, eval_from_ckpt):
  """Returns a list of all checkpoints after and including 'eval_from_ckpt'.
  Args:
    dir_path: Path to checkpoints from training.
    eval_from_ckpt: The checkpoint to start evaluating from (including).
  Raises:
    ValueError: if the specified checkpoint is not an integer.
  """
  ...
  try:
    ...
    raise ValueError(
      'Expected int, but got \'{}\' as checkpoint number. Set to 0 if all checkpoints should be evaluated.'.format(
        eval_from_ckpt))

  for ckpt_file in ckpt_files:
    ckpt_id = ckpt_file.split('-')[-1].split('.')[0]
    if int(ckpt_id) >= int(eval_from_ckpt) and ckpt_id not in ckpt_list:
      ckpt_list.append(ckpt_id)

  ckpt_list.sort(key=int)
...
def _run_checkpoint_once(tensor_dict,
  ...
  Args:
    ...
    current_checkpoint: Holds the current checkpoint if evaluating multiple checkpoints.
  ...
  if restore_fn and eval_from_ckpt == '':
    restore_fn(sess)
  else:
...
def repeated_checkpoint_run(tensor_dict,
  ...
  ckpt_eval_list = []
  if eval_from_ckpt != '':
    ckpt_eval_list = return_checkpoints(checkpoint_dirs[0], eval_from_ckpt)
  ...
  while True:
    ...
    if len(ckpt_eval_list) > 0 and ckpt_index < len(ckpt_eval_list):
      model_path = checkpoint_dirs[0] + "\model.ckpt-" + ckpt_eval_list[ckpt_index]
    ...
    if not model_path:
      ...
      global_step, metrics = _run_checkpoint_once(tensor_dict, evaluators,
                                                  ...
                                                  master, model_path, eval_from_ckpt,
                                                  save_graph, save_graph_dir,
