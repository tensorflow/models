...
class TfExampleDecoder(data_decoder.DataDecoder):
    ...
    self.items_to_handlers = {
        ...
        fields.InputDataFields.groundtruth_caption: (
            slim_example_decoder.ItemHandlerCallback(
              ['image/caption/string', 'image/caption/offset', 'image/caption/length'], 
              self._sample_caption)),
    }
  ...
  def _sample_caption(self, keys_to_tensors):
    """Samples a caption for an image.

    Args:
      keys_to_tensors: a dictionary from keys to tensors.

    Returns:
      caption_string: A 1-D string tensor of shape [caption_length].
    """
    def _sparse_to_dense_helper(x, default_value):
      if isinstance(x, tf.SparseTensor):
        x = tf.sparse_tensor_to_dense(x, default_value)
      return x

    caption_strings = _sparse_to_dense_helper(
        keys_to_tensors['image/caption/string'], default_value="")
    caption_offsets = _sparse_to_dense_helper(
        keys_to_tensors['image/caption/offset'], default_value=0)
    caption_lengths = _sparse_to_dense_helper(
        keys_to_tensors['image/caption/length'], default_value=0)

    index = tf.random_uniform(shape=[], 
        maxval=tf.shape(caption_offsets)[0], dtype=tf.int32)
    (offset, length) = caption_offsets[index], caption_lengths[index]
    return caption_strings[offset: offset + length]
