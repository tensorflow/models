...
class SSDMetaArch(model.DetectionModel):
  ...
  def restore_map(self,
                  ...
                  load_all_optimizer_checkpoint_vars=True,
                  load_all_moving_average_checkpoint_vars=True,
    ...
    Args:
      ...
      load_all_detection_checkpoint_vars: whether to load all detection
        variables (when `fine_tune_checkpoint_type` is `detection`). If
        False, only variables within the feature extractor scopes are
        included. Default False.
      load_all_optimizer_checkpoint_vars: whether to load all optimizer
        variables. If False, only variables themselves are included(maybe
        their moving average variables in addition).
        otherwise, all variables including derivative variables generated
        by optimizers like rmsprop, adam will be included. Default True
        (switched to true for keeping behavier).
      load_all_moving_average_checkpoint_vars: whether to load all moving
        average variables of the trained parametersvariables. If False,
        only variables themselves are included(maybe their optimizer
        variables in addition). otherwise, all variables including
        derivative variables generated by ema optimizer wrapper will be
        included. Default True
        (switched to true for keeping behavier).
      use_moving_average: whether to use the moving average of the trained
        parameters, fetch from proto config file.
      training_optimizer: training optimizer instance constructed by optimizer
        builder. maybe has a MovingAverageOptimizer wrapper if use_moving_average
        is true.
    ...
    include_patterns = ['']
    exclude_patterns = []
    if not load_all_optimizer_checkpoint_vars:
      exclude_patterns.append(training_optimizer._optimizer._name)
    if use_moving_average:
      if not load_all_moving_average_checkpoint_vars:
        exclude_patterns.append(training_optimizer._ema._name)
    if not load_all_detection_checkpoint_vars:
      include_patterns=[self._extract_features_scope]
    variables_to_restore_filtered  = tf.contrib.framework.filter_variables(
      variables_to_restore, include_patterns=include_patterns,
      exclude_patterns=exclude_patterns)
    if fine_tune_checkpoint_type == 'classification':
      return {re.split('^' + self._extract_features_scope + '/',
                         var.op.name)[-1]: var for var in variables_to_restore_filtered}
    else:
      return {var.op.name: var for var in variables_to_restore_filtered}