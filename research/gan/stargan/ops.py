...
def condition_input_with_pixel_padding(input_tensor, condition_tensor):
  ...
  Raises:
    InvalidArgumentError: Raise InvalidArgumentErro if the batch_size is
    different between input_tensor and condition_tensor.
  ...
  with tf.control_dependencies([
      tf.assert_equal(
          ...
          message='Input has different batch size as condition: {} vs {}.'.
          format(input_tensor.shape[0], condition_tensor.shape[0]))
  ]):
