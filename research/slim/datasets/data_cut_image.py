...
def cutImage(srcPath, dstPath, logo_width=None, logo_height=None, x_padding=0, y_padding=0):
    count = 0
    total_count = os.listdir(srcPath)
    for filename in os.listdir(srcPath):
        ...
        if os.path.isfile(srcFile):
            with Image.open(srcFile) as im:
                ...
                count = count + 1
                sys.stdout.write('\r>> Converting image %d/%d' % (count, len(total_count)))
                sys.stdout.flush()
...
if __name__ == '__main__':
    # gain_train_data()
    class_dir_name = "SmallLogo"

    srcPath = "E:\\data_mining\\data\\east_ic_logo\\train\\" + class_dir_name
    dstPath = "E:\\data_mining\\data\\east_ic_logo\\train_slim\\east_ic_logo\\" + class_dir_name
