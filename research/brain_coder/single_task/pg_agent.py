...
class LMAgent(object):
  ...
  def __init__(self, global_config, task_id=0,
    ...
    if not self.a2c:
      tf.logging.info('Using exponential moving average REINFORCE baselines.')
      self.ema_baseline_decay = config.ema_baseline_decay
      self.ema_by_len = [0.0] * global_config.timestep_limit
    else:
      tf.logging.info('Using advantage (a2c) with learned value function.')
    ...
    if config.topk and config.topk_loss_hparam:
      ...
      tf.logging.info('Made max-priorty-queue with capacity %d',
                   self.top_episodes.capacity)
    else:
      self.top_episodes = None
      self.topk_loss_hparam = 0.0
      tf.logging.info('No max-priorty-queue')
    ...
    tf.logging.info('num_replay_per_batch: %d', self.num_replay_per_batch)
    tf.logging.info('num_on_policy_per_batch: %d', self.num_on_policy_per_batch)
    tf.logging.info('replay_alpha: %s', self.replay_alpha)
    if self.num_replay_per_batch > 0:
      ...
      tf.logging.info('Took %s sec to load replay buffer from disk.',
                   int(time.time() - start_time))
      tf.logging.info('Replay buffer file location: "%s"',
    ...
    if self.top_episodes is not None:
      ...
      tf.logging.info('Including off policy loss.')
    else:
  ...
  def update_step(self, session, rl_batch, train_op, global_step_op,
    ...
    if num_programs_from_policy:
      ...
      if max_tot_r >= self.top_reward:
        ...
        tf.logging.info('Top code: r=%.2f, \t%s', max_tot_r, code_strings[max_i])
      if self.top_episodes is not None:
