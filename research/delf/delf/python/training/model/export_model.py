...
def main(argv):
  ...
  with tf.Graph().as_default() as g, tf.Session(graph=g) as sess:
    ...
    input_abs_thres = tf.placeholder(
        tf.float32, shape=(), name='input_abs_thres')
    input_scales = tf.placeholder(tf.float32, shape=[None], name='input_scales')
    input_max_feature_num = tf.placeholder(
        tf.int32, shape=(), name='input_max_feature_num')

    extracted_features = extractor.ExtractLocalFeatures(
