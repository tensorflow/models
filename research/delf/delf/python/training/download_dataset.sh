...
validate_md5_checksum() {
    ...
    if [[ "$content_md5" != "" && "$content_md5" = "$expected_md5" ]]; then
        ...
        echo "Check failed. MD5 checksums don't match."
    fi
}
...
download_image_file() {
    # Downloads one image files of a split and untar it.
    local split=$1
...
download_split() {
    # Downloads all artefacts, metadata CSV files and image files of a single split.
    local split=$1
...
download_all_splits() {
    # Downloads all artefacts, metadata CSV fils and image files of all splits.
