  ...
  if not tf.io.gfile.exists(dst_dir):
    ...
    with tarfile.open(dst_file, 'r:gz') as tar:
      for member in tar.getmembers():
        member_path = os.path.realpath(os.path.join(dst_dir, member.name))
        if not member_path.startswith(dst_dir_real + os.sep) and \
            member_path != dst_dir_real:
          raise ValueError(
              'Unsafe tar member path detected: {}'.format(member.name))
