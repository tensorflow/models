...
class Model(object):
  ...
  def conv_tower_fn(self, images, is_training=True, reuse=None):
    ...
    tf.logging.debug('Using final_endpoint=%s', mparams.final_endpoint)
  ...
  def _create_lstm_inputs(self, net):
    ...
    if num_features < self._params.seq_length:
      ...
      tf.logging.warning('Ignoring some features: use %d of %d (shape=%s)',
  ...
  def create_init_fn_to_restore(self, master_checkpoint,
    ...
    def assign_from_checkpoint(variables, checkpoint):
      tf.logging.info('Request to re-store %d weights from %s',
                   len(variables), checkpoint)
      if not variables:
        tf.logging.error('Can\'t find any variables to restore.')
        sys.exit(1)
    ...
    def init_assign_fn(sess):
      tf.logging.info('Restoring checkpoint(s)')
